...
You can hide the matrix by クリックing on the _Close_ button (the right-most
...
When the _Rhythm Ruler_ block is クリックed, the _Rhythm Ruler_ ウィジェット is
...
three third-notes. クリックing on the _Play_ button to the left of each row
...
In the above example, the _Major_ mode has been 転回ed, transforming
it into _Phrygian_.
...
When _Pitch Staircase_ block is クリックed, the _Pitch Staircase_ ウィジェット is
...
クリックing on the _Play_ button to the left of each row will playback
...
When the _ピッチ・スライダー_ block is クリックed, the _ピッチ・スライダー_ ウィジェット is
...
クリックing in a column will extact the corresponding _Note_ blocks, for example:
...
visualize テンポ, defined in 拍子s per minute (BPM). When the _テンポ_ block
is クリックed, the _テンポ_ ウィジェット is initialized.

The _Master 泊s per Minute_ block contained in the clamp of the
