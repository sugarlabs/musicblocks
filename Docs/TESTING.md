...
- **Scoped Teardown & Exception Filtering**: Uses precise title-based and class-based selector scoping (`.wftTitle contains("status")` and `.mb-system-dialog`) for window/dialog cleanup to avoid cross-spec leakage. The spec exception handler uses a targeted allowlist for known benign runtime variances (such as ResizeObserver loop limits and teardown timing variances) while permitting genuine application exceptions to fail the suite.
