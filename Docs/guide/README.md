...
The next block is specified using a _Pitch-number_ block (the `7th semitone` above `C` in `Octave 4` is `G`). The offset for the pitch
...
double-dotted quarter note will play for `7/16` `(i.e. 1/4 + 1/8 + 1/16)` of a beat (which is the same as `4/16 + 2/16 + 1/16 = 7/16`).
...
be played as a `1/12` note (`1/8 + 1/24 === 1/6` and `1/8 - 1/24 === 1/12`). Observe that the total duration of the pair of notes is
unchanged.
...
put the highest note on top, the first pitch is the `Sol` in `Octave 5`. From there, we use `-1` as the argument to the _Scalar step_ block
inside the _Repeat_, working our way down to `Sol` in `Octave 4`. Another detail to note is the use of the _Mode length_ block.
...
piano. The intervals in the _Major_ mode are `2, 2, 1, 2, 2, 2, 1`. Many other common modes are built into Music Blocks, including, of
