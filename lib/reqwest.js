...
if(r){switch(type){case'json':try{resp=JSON.parse(r)}catch(err){return error(resp,'Could not parse JSON in response',err)}
break
case'js':return error(resp,'JavaScript responses are not supported for security reasons',new Error('JavaScript response execution disabled'))
break
case'html':resp=r
break
