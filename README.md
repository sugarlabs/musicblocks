...
This section helps resolve common issues users may encounter while installing, running, or using Music Blocks.

### Port 3000 Is Already in Use
Problem:
EADDRINUSE: address already in use error while starting the server.
...
Problem:
The application loads but shows a blank screen or missing blocks.
...
Problem:
Application runs but produces no sound.

Solution:
Check browser audio permissions
Ensure system volume is not muted
Click once on the page to enable audio

### Docker Container Won’t Start
Problem:
Docker build or container startup fails.
...
Problem:
Linting errors block the build.
...
Problem:
The application feels slow or becomes unresponsive.

Solution:
Restart the development server
Close background applications
Use updated Node.js and browser
...
Open a new issue with logs and screenshots

### Support and Documentation
Issues: https://github.com/sugarlabs/musicblocks/issues
