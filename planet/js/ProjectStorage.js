...
class ProjectStorage {
    constructor(Planet) {
        ...
        this.maxVersions = 10; // Maximum versions to keep per project
    ...
    async initialiseStorage() {
        ...
        if (this.data.VersionHistory === null || this.data.VersionHistory === undefined) {
            this.data.VersionHistory = {};
        }
        this.fireDataLoaded();
        await this.save();
    }
    ...
    /**
     * Save current project state as a new version
     * @param {string} description - Optional description for this version
     * @returns {string} Version ID
     */
    async saveVersion(description = null) {
        const projectId = this.data.CurrentProject;
        if (!projectId || !this.data.Projects[projectId]) {
            throw new Error("No active project to version");
        }

        const project = this.data.Projects[projectId];
        const versionId = this.generateID();
        
        // Initialize version history for this project if needed
        if (!this.data.VersionHistory[projectId]) {
            this.data.VersionHistory[projectId] = [];
        }
        ...
        const version = {
            id: versionId,
            timestamp: Date.now(),
            description: description || `Version ${this.data.VersionHistory[projectId].length + 1}`,
            projectData: project.ProjectData,
            projectImage: project.ProjectImage,
            projectName: project.ProjectName
        };

        this.data.VersionHistory[projectId].push(version);

        if (this.data.VersionHistory[projectId].length > this.maxVersions) {
            this.data.VersionHistory[projectId].shift(); // Remove oldest
        }
    ...
    /**
     * Get version history for current or specified project
     * @param {string} projectId - Optional project ID (defaults to current)
     * @returns {Array} Array of version objects with metadata
     */
    getVersionHistory(projectId = null) {
        const id = projectId || this.data.CurrentProject;
        if (!this.data.VersionHistory[id]) {
            return [];
        }

        return this.data.VersionHistory[id].map(v => ({
            id: v.id,
     ...
     * @param {boolean} createBackup - Create backup of current state before reverting
     * @returns {boolean} Success status
     */
    async revertToVersion(versionId, createBackup = true) {
        const projectId = this.data.CurrentProject;
        if (!projectId || !this.data.VersionHistory[projectId]) {
            throw new Error("No version history found");
        }

        const version = this.data.VersionHistory[projectId].find(v => v.id === versionId);
        if (!version) {
            throw new Error("Version not found");
        }

        if (createBackup) {
            await this.saveVersion("Auto-backup before revert");
        }

        this.data.Projects[projectId].ProjectData = version.projectData;
        this.data.Projects[projectId].ProjectImage = version.projectImage;
        this.data.Projects[projectId].ProjectName = version.projectName;
        this.data.Projects[projectId].DateLastModified = Date.now();
    ...
    async deleteVersion(versionId) {
        ...
        const index = this.data.VersionHistory[projectId].findIndex(v => v.id === versionId);
        if (index === -1) {
            return false;
        }

        this.data.VersionHistory[projectId].splice(index, 1);
        await this.save();
        return true;
    }
     ...
     * @param {string} versionId - ID of version to retrieve
    ...
    getVersion(versionId) {
        ...
        return this.data.VersionHistory[projectId].find(v => v.id === versionId) || null;
    }

    /**
     * Clear all version history for current project
     * @returns {boolean} Success status
     */
