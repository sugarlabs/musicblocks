...
function ModeWidget() {
    ...
    this.__playNextNote = function(time, noteCounter) {
        ...
        setTimeout(function() {
            ...
            that._logo.synth.trigger(noteToPlay[0].replace(/♯/g, '#').replace(/♭/g, 'b') + noteToPlay[1], that._noteValue, 'poly', null);
    ...
    this._playNote = function(idx) {
        ...
        if (cell.style.backgroundColor === 'black') {
            ...
            this._logo.synth.trigger(noteToPlay[0].replace(/♯/g, '#').replace(/♭/g, 'b') + noteToPlay[1], this._noteValue, 'poly', null);
        }
    };
