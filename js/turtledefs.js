...
if (GUIDEURL === 'guide url' || GUIDEURL === '') {
    GUIDEURL = 'https://github.com/sugarlabs/musicblocks/tree/master/guide/README.md';
}
...
function beginnerBlock(name) {
 // Only these blocks appear on the palette in beginner mode.
    return ['newnote', 'note4', 'rest2', 'mynotevalue',  // notes palette
           ...
           'status', 'matrix', 'rhythmruler2', 'pitchslider', 'rhythm2', 'stuplet', 'musickeyboard', 'tempo', 'modewidget', 'matrixcmajor', 'matrixgmajor', 'chromatic', // widgets palette
...
function createHelpContent() {
	...
	console.log(TITLESTRING);
        ...
        HELPCONTENT = [
            ...
            [_('New project'), _('Initialize a new project.'), 'data:image/svg+xml;base64,' + window.btoa(unescape(encodeURIComponent(NEWBUTTON)))],
            [_('Load project from file'), _('You can also load projects from the file system.'), 'data:image/svg+xml;base64,' + window.btoa(unescape(encodeURIComponent(LOADBUTTON)))],
            [_('save'), _('Save project') + ': ' + _('Save your project to a file.') + '<br/><br/>' + _('Save mouse artwork as SVG') + ': '  + _('Save graphics from your project to as SVG.') + '<br/><br/>' + _('Save mouse artwork as PNG') + ': ' + _('Save graphics from your project as PNG.') + '<br/><br/>' + _('Save music as WAV') + ': ' + _('Save audio from your project as WAV.') + '<br/><br/>' + _('Save sheet music as ABC') + ': ' + _('Save your project to as an ABC file.') + '<br/><br/>' + _('Save sheet music as Lilypond') + ': ' + _('Save your project to as a Lilypond file.') + '<br/><br/>' + _('Save block artwork as SVG') + ': ' + _('Save block artwork as an SVG file.'), 'data:image/svg+xml;base64,' + window.btoa(unescape(encodeURIComponent(SAVEBUTTON)))],
    ...
    // Each BLOCKHELP entry in the dictionary contains a help string,
    // the path of the help svg, an override name for the help svg
    // file, and an optional macro name for generating the help
    // output.
    BLOCKHELP = {
        // Rhythm palette
        'newnote': [_('The Note block is a container for one or more Pitch blocks.') + ' ' + _('The Note block specifies the duration (note value) of its contents.'), 'documentation', null, 'note1'],
        'playdrum': [_('You can use multiple Drum blocks within a Note block.'), 'documentation', null, 'note4'],
        'rest2': [_('A rest of the specified note value duration can be constructed using a Silence block.'), 'documentation', null, 'rest2'],
        'mynotevalue': [_('The Note value block is the value of the duration of the note currently being played.'), 'documentation', null, 'everybeathelp'],
        // Meter palette
        'meter': [_('The beat of the music is determined by the Meter block (by default, 4 1/4 notes per measure).'), 'documentation', null, 'meter'],
        'setmasterbpm2': [_('The Master beats per minute block sets the number of 1/4 notes per minute for every voice.'), 'documentation', null, 'setmasterbpm2'],
        'setbpm3': [_('The Beats per minute block sets the number of 1/4 notes per minute.'), 'documentation', null, 'bpmhelp'],
        'everybeatdo': [_('The On-every-note block let you specify actions to take on every note.'), 'documentation', null, 'everybeathelp'],
        'beatvalue': [_('The Beat count block is the number of the current beat,') + ' ' + _('eg 1, 2, 3, or 4.') + ' ' + _('In the figure, it is used to take an action on the first beat of each measure.'), 'documentation', null, 'beatvaluehelp'],
        'elapsednotes2': [_('The Notes played block is the number of notes that have been played.') + ' ' + _('(By default, it counts quarter notes.)'), 'documentation', null, 'everybeathelp'],
        // Pitch palette
        'pitch': [_('The Pitch block specifies the pitch name and octave of a note that together determine the frequency of the note.'), 'documentation', null, 'note1'],
        'solfege': [_('Pitch can be specified in terms of do re mi fa sol la ti.'), 'documentation', null, 'note1'],
        'notename': [_('Pitch can be specified in terms of C D E F G A B.'), 'documentation', null, 'note2'],
        'pitchnumber': [_('The Pitch Number block will play a pitch associated by its number eg 0 for C and 7 for G.'), 'documentation', null, 'note5'],
        'steppitch': [_('The Scalar Step block (in combination with a Number block) will play the next pitch in a scale,') + ' ' + _('eg if the last note played was sol, Scalar Step 1 will play la.'), 'documentation', ''],
        'hertz': [_('The Hertz block (in combination with a Number block) will play a sound at the specified frequency.'), 'documentation', null, 'note3'],
        'setscalartransposition': [_('The Scalar transposition block will shift the pitches contained inside Note blocks up (or down) the scale.') + ' ' + _('In the example shown above, sol is shifted up to la.'), 'documentation', null, 'scalartranshelp'],
        'pitchinhertz': [_('The Pitch in Hertz block is the value in Hertz of the pitch of the note currently being played.'), 'documentation', ''],
        'mypitch': [_('The Pitch number block is the value of the pitch of the note currently being played.'), 'documentation', null, 'everybeathelp'],
        // Intervals palette
        'setkey2': [_('The Set key block is used to set the key and mode,') + ' ' + _('eg C Major'), 'documentation', null, 'movablehelp'],
        'modelength': [_('The Mode length block is the number of notes in the current scale.') + ' ' + _('Most Western scales have 7 notes.'), 'documentation', ''],
	...
	// Ornament palette
        'neighbor2': [_('The Neighbor block rapidly switches between neighboring pitches.'), 'documentation', null, 'neighbor2help'],
        'neighbor': [_('The Neighbor block rapidly switches between neighboring pitches.'), 'documentation', null, 'neighborhelp'],
        // Volume palette
        ...
        'setsynthvolume': [_('The Set synth volume block will change the volume of a particular synth,') + ' ' + _('eg guitar violin snare drum etc.') + ' ' + _('The default volume is 50.') + ' ' + _('The range is 0 for silence to 100 for full volume.'), 'documentation', ''],
        'setnotevolume': [_('The Set master volume block sets the volume for all synthesizers.'), 'documentation', ''],
        // Drum palette
        ...
        'rhythmruler2': [_('The Rhythm Maker block opens a tool to create drum machines.'), 'documentation', null, 'rhythmruler2'],
        'pitchslider': [_('The Pitch-slider block opens a tool to generate arbitray pitches.'), 'documentation', null, 'pitchslider'],
        'rhythm2': [_('The Rhythm block is used to generate rhythm patterns.'), 'documentation', null, 'rhythm2'],
        'stuplet': [_('Tuplets are a collection of notes that get scaled to a specific duration.') + ' ' + _('Using tuplets makes it easy to create groups of notes that are not based on a power of 2.'), 'documentation', null, 'matrix'],
        'musickeyboard': [_('The Music keyboard block opens a piano keyboard that can be used to create notes.'), 'documentation', null, 'musickeyboard2'],
        'tempo': [_('The Tempo block opens a metronome to visualize the beat.'), 'documentation', null, 'tempo'],
        'modewidget': [_('The Custom mode block opens a tool to explore musical mode (the spacing of the notes in a scale).'), 'documentation', ''],
	'meterwidget': [_('The Meter block opens a tool to select strong beats for the meter.'), 'documentation', null, 'meterwidget'],
        // Flow palette
        'repeat': [_('The Repeat block will repeat the contained blocks.') + ' ' + _('In this example the note will be played 4 times.'), 'documentation', null, 'repeathelp'],
        'forever': [_('The Forever block will repeat the contained blocks forever.') + ' ' + _('In this example of a simple drum machine a kick drum will play 1/4 notes forever.'), 'documentation', null, 'foreverhelp'],
        'if':  [_('Conditionals lets your program take different actions depending on the condition.') + ' ' + _('In this example if the mouse button is pressed a snare drum will play.'), 'documentation', null, 'ifhelp'],
        'ifthenelse': [_('Conditionals lets your program take different actions depending on the condition.') + ' ' + _('In this example if the mouse button is pressed a snare drum will play, else a kick drum will play.'), 'documentation', null, 'elifhelp'],
        'backward': [_('The Backward block runs code in reverse order (Musical retrograde).'), 'documentation', ''],
        // Action palette
        'action': [_('The Action block is used to group together blocks so that they can be used more than once.') + ' ' + _('It is often used for storing a phrase of music that is repeated.'), 'documentation', null, 'actionhelp'],
        'start': [_('Each Start block is a separate voice.') + ' ' + _('All of the Start blocks run at the same time when the Play button is pressed.'), 'documentation', ''],
        'listen': [_('The Listen block is used to listen for an event such as a mouse click.') + ' ' + _('When the event happens, an action is taken.'), 'documentation', null, 'broadcasthelp'],
        'dispatch': [_('The Broadcast block is used to trigger an event.'), 'documentation', null, 'broadcasthelp'],
        'do': [_('The Do block is used to initiate an action.') + ' ' + _('In the example, it is used with the One of block to choose a random phase.'), 'documentation', null, 'dohelp'],
        // Boxes palette
        'storebox1': [_('The Store in Box 1 block is used to store a value in Box 1.'), 'documentation', null, 'box1help'],
        'box1': [_('The Box 1 block returns the value stored in Box 1.'), 'documentation', null, 'box1help'],
        'storebox2': [_('The Store in Box 2 block is used to store a value in Box 2.'), 'documentation', null, 'box2help'],
        'box2': [_('The Box 2 block returns the value stored in Box 2.'), 'documentation', null, 'box2help'],
        'increment': [_('The Add-to block is used to add to the value stored in a box.') + ' ' + _('It can also be used with other blocks such as Color and Pen size.'), 'documentation', ''],
        'incrementOne': [_('The Add-1-to block adds one to the value stored in a box.'), 'documentation', ''],
        // Number palette
        'number': [_('The Number block holds a number.'), 'documentation', ''],
        'random': [_('The Random block returns a random number.'), 'documentation', null, 'randomhelp'],
        'oneOf': [_('The One-of block returns one of two choices.'), 'documentation', null, 'oneofhelp'],
        'plus': [_('The Plus block is used to add.'), 'documentation', ''],
        'minus': [_('The Minus block is used to subtract.'), 'documentation', ''],
        'multiply': [_('The Multiply block is used to multiply.'), 'documentation', ''],
        'divide': [_('The Divide block is used to divide.'), 'documentation', null, 'note1'],
        // Boolean palette
        'greater': [_('The Greater-than block returns True if the top number is greater than the bottom number.'), 'documentation', ''],
        'less': [_('The Less-than block returns True if the top number is less than the bottom number.'), 'documentation', ''],
        'equal': [_('The Equal block returns True if the two numbers are equal.'), 'documentation', ''],
        // Mouse palette
        'forward': [_('The Forward block moves the mouse forward.'), 'documentation', null, 'forwardhelp'],
        'back': [_('The Back block moves the mouse backward.'), 'documentation', null, 'forwardhelp'],
        'left': [_('The Left block turns the mouse to the left.'), 'documentation', null, 'forwardhelp'],
        'right': [_('The Right block turns the mouse to the right.'), 'documentation', null, 'forwardhelp'],
        'arc': [_('The Arc block moves the mouse in a arc.'), 'documentation', null, 'archelp'],
        'setxy': [_('The Set XY block moves the mouse to a specific position on the screen.'), 'documentation', ''],
        'scrollxy': [_('The Scroll XY block moves the canvas.'), 'documentation', null, 'everybeathelp'],
        'x': [_('The X block returns the horizontal position of the mouse.'), 'documentation', null, 'xyhelp'],
        'y': [_('The Y block returns the vertical position of the mouse.'), 'documentation', null, 'xyhelp'],
        'heading': [_('The Heading block returns the orientation of the mouse.'), 'documentation', ''],
        // Pen palette
        'setpensize': [_('The Set-pen-size block changes the size of the pen.'), 'documentation', ''],
        'penup': [_('The Pen-up block raises the pen so that it does not draw.'), 'documentation', ''],
        'pendown': [_('The Pen-down block lowers the pen so that it draws.'), 'documentation', ''],
        'color': [_('The Color block returns the current pen color.'), 'documentation', ''],
        'setcolor': [_('The Set-color block changes the pen color.'), 'documentation', ''],
        'setshade': [_('The Set-shade block changes the pen color from dark to light.'), 'documentation', ''],
        // Media palette
        'print': [_('The Print block displays text at the top of the screen.'), 'documentation', ''],
        'text': [_('The Text block holds a text string.'), 'documentation', ''],
        'media': [_('The Media block is used to import an image.'), 'documentation', null, 'turtleshell'],
        'show': [_('The Show block is used to display text or images on the canvas.'), 'documentation', ''],
        'turtleshell': [_('The Shell block is used to change the appearance of the mouse.'), 'documentation', null, 'turtleshell'],
        'speak': [_('The Speak block outputs to the text-to-speech synthesizer'), 'documentation', ''],
        'height': [_('The Height block returns the height of the canvas.'), 'documentation', ''],
        'width': [_('The Width block returns the width of the canvas.'), 'documentation', ''],
        'toppos': [_('The Top block returns the position of the top of the canvas.') + ' ' + _('In this example, the mouse moves upward until it reaches the top edge of the canvas; then it reappears at the bottom of the canvas.'), 'documentation', null, 'bottomposhelp'],
        'bottompos': [_('The Bottom block returns the position of the bottom of the canvas.') + ' ' + _('In this example, the mouse moves upward until it reaches the top edge of the canvas; then it reappears at the bottom of the canvas.'), 'documentation', null, 'bottomposhelp'],
        'leftpos': [_('The Left block returns the position of the left of the canvas.') + ' ' + _('In this example, the mouse moves right until it reaches the right edge of the canvas; then it reappears at the left of the canvas.'), 'documentation', null, 'lrhelp'],
        'rightpos': [_('The Right block returns the position of the right of the canvas.') + ' ' + _('In this example, the mouse moves right until it reaches the right edge of the canvas; then it reappears at the left of the canvas.'), 'documentation', null, 'lrhelp'],
        // Sensors palette
        'mousebutton': [_('The Mouse-button block returns True if the mouse button is pressed.'), 'documentation', null, 'mousebuttonhelp'],
        'mousex': [_('The Cursor X block returns the horizontal position of the mouse.'), 'documentation', null, 'mousebuttonhelp'],
        'mousey': [_('The Cursor Y block returns the vertical position of the mouse.'), 'documentation', null, 'mousebuttonhelp'],
        'myclick': [_('The Click block returns True if a mouse has been clicked.'), 'documentation', null, 'clickhelp'],
        // Mice palette
        'setturtlename2': [_('The Set-name block is used to name a mouse.'), 'documentation', null, 'clickhelp'],
        'turtlename': [_('The Mouse-name block returns the name of a mouse.'), 'documentation', null, 'clickhelp'],
        // Advanced blocks
        // Rhythm palette
        'rhythmicdot2': [_('The Dot block extends the duration of a note by 50%.') + ' ' + _('Eg a dotted quarter note will play for 3/8 (1/4 + 1/8) of a beat.'), 'documentation', null, 'dothelp'],
        'tie': [_('The Tie block works on pairs of notes, combining them into one note.'), 'documentation', null, 'tiehelp'],
        'multiplybeatfactor': [_('The Multiply note value block changes the duration of notes by changing their note values.'), 'documentation', null, 'multiplybeathelp'],
        'skipnotes': [_('The Skip notes block will cause notes to be skipped.'), 'documentation', null, 'skiphelp'],
        'newswing2': [_('The Swing block works on pairs of notes (specified by note value), adding some duration (specified by swing value) to the first note and taking the same amount from the second note.'), 'documentation', null, 'swinghelp'],
        'osctime': [_('The Milliseconds block is similar to a Note block except that it uses time (in MS) to specify the note duration.'), 'documentation', null, 'osctimehelp'],
        // Meter palette
        'pickup': [_('The Pickup block is used to accommodate any notes that come in before the beat.'), 'documentation', null, 'pickup'],
        // 'bpm': [_('The Beats per minute block changes the beats per minute of any contained notes.'), 'documentation', ''],
        'onbeatdo': [_('The On-strong-beat block let you specify actions to take on specified beats.'), 'documentation', null, 'everybeathelp'],
        'offbeatdo': [_('The On-weak-beat block let you specify actions to take on weak (off) beats.'), 'documentation', null, 'everybeathelp'],
        'drift': [_('The No clock block decouples the notes from the master clock.'), 'documentation', ''],
        'elapsednotes': [_('The Whole notes played block returns the total number of whole notes played.'), 'documentation', null, 'elapsedhelp'],
        'notecounter': [_('The Note counter block can be used to count the number of contained notes.'), 'documentation', null, 'notecounterhelp'],
        'measurevalue': [_('The Measure count block returns the current measure.'), 'documentation', ''],
        'bpmfactor': [_('The Beats per minute block returns the current beats per minute.'), 'documentation', ''],
        // 'beatfactor': [_(''), 'documentation', ''],
        // pitch palette
        'accidental': [_('The Accidental block is used to create sharps and flats'), 'documentation', null, 'accidental'],
        'settransposition': [_('The Semi-tone transposition block will shift the pitches contained inside Note blocks up (or down) by half steps.') + ' ' + _('In the example shown above, sol is shifted up to sol#.'), 'documentation', ''],
        'register': [_('The Register block provides an easy way to modify the register (octave) of the notes that follow it.'), 'documentation', null, 'registerhelp'],
        'invert1': [_('The Invert block rotates any contained notes around a target note.'), 'documentation', null, 'inverthelp'],
        'deltapitch': [_('The Change in pitch block is the difference (in half steps) between the current pitch being played and the previous pitch played.'), 'documentation', null, 'deltapitchhelp'],
        // 'customNote': ['', 'documentation', ''],
        //.TRANS: 'ni', 'dha', 'pa', 'ma', 'ga', 're', 'sa' are East Indian note names.
        'eastindiansolfege': [_('Pitch can be specified in terms of ni dha pa ma ga re sa.'), 'documentation', null, 'eihelp'],
        'accidentalname': [_('The Accidental selector block is used to choose between double-sharp, sharp, natural, flat, and double-flat.'), 'documentation', ''],
        'number2octave': [_('The Number to octave block will convert a pitch number to an octave.'), 'documentation', ''],
        'setpitchnumberoffset': [_('The Set pitch number offset block is used to set the offset for mapping pitch numbers to pitch and octave.'), 'documentation', null, 'pitchnumberhelp'],
        'consonantstepsizeup': [_('The Scalar step up block returns the number of semi-tones up to the next note in the current key and mode.'), 'documentation', ''],
        ...
        // Intervals palette
        'movable': [_('When Moveable do is false, the solfege note names are always tied to specific pitches,') + ' ' + _('eg "do" is always "C-natural"); when Moveable do is true, the solfege note names are assigned to scale degrees ("do" is always the first degree of the major scale).'), 'documentation', null, 'movablehelp'],
        'definemode': [_('The Define mode block allows you define a custom mode by specifiying pitch numbers.'), 'documentation', null, 'definemode'],
        'semitoneinterval': [_('The Semi-tone interval block calculates a relative interval based on half steps.') + ' ' + _('In the figure, we add sol# to sol.'), 'documentation', ''],
        'measureintervalscalar': [_('The Scalar interval block measures the distance between two notes in the current key and mode.'), 'documentation', ''],
        ...
        'tremolo': [_('The Tremolo block adds a wavering effect.'), 'documentation', null, 'tremolohelp'],
        'harmonic2': [_('The Harmonic block will add harmonics to the contained notes.'), 'documentation', null, 'harmonichelp'],
        'harmonic': [_('The Weighted partials block is used to specify the partials associated with a timbre.'), 'documentation', ''],
        'partial': [_('The Partial block is used to specify a weight for a specific partical harmonic.'), 'documentation', ''],
        'fmsynth': [_('The FM synth block is a frequency modulator used to define a timbre.'), 'documentation', null, 'fmsynthhelp'],
        'amsynth': [_('The AM synth block is an amplitude modulator used to define a timbre.'), 'documentation', null, 'amsynthhelp'],
        ...
        'effectsname': [_('The Effects name block is used to select a sound effect.'), 'documentation', null, 'effectshelp'],
        'noisename': [_('The Noise name block is used to select a noise synthesizer.'), 'documentation', ''],
        // Widgets palette
        'pitchslider': [_('The Pitch slider tool to is used to generate pitches at selected frequencies.'), 'documentation', ''],
        'pitchstaircase': [_('The Pitch staircase tool to is used to generate pitches from a given ratio.'), 'documentation', null, 'pitchstaircase'],
        'pitchdrummatrix': [_('The Pitch drum matrix is used to map pitches to drum sounds.'), 'documentation', ''],
        'temperament': [_('The Temperament tool is used to define custom tuning.'), 'documentation', null, 'temperament'],
        'temperamentname': [_('The Temperament name block is used to select a tuning method.'), 'documentation', ''],
        'tuplet4': [_('The Tuplet block is used to generate a group of notes played in a condensed amount of time.'), 'documentation', null, 'tuplet4'],
        // Flow palette
        'while': [_('The While block will repeat while the condition is true.'), 'documentation', null, 'whilehelp'],
        'until': [_('The Until block will repeat until the condition is true.'), 'documentation', null, 'untilhelp'],
        'waitfor': [_('The Waitfor block will wait until the condition is true.'), 'documentation', null, 'waitforhelp'],
        'break': [_('The Stop block will stop a loop') + ': ' + _('Forever, Repeat, While, or Until.'), 'documentation', ''],
        'switch': [_('The Switch block will run the code in the matching Case.'), 'documentation', null, 'switchhelp'],
        'case': [_('The Case block is used inside of a Switch to define matches.'), 'documentation', null, 'switchhelp'],
        'default': [_('The Default block is used inside of a Switch to define a default action.'), 'documentation', null, 'switchhelp'],
        'duplicatenotes': [_('The Duplicate block will run each block multiple times.') + ' ' + _('The output of the example is: Sol, Sol, Sol, Sol, Re, Re, Re, Re, Sol, Sol, Sol, Sol.'), 'documentation', null, 'duphelp'],
        // Action palette
        'arg': [_('The Arg block contains the value of an argument passed to an action.'), 'documentation', ''],
        'namedarg': [_('The Arg block contains the value of an argument passed to an action.'), 'documentation', ''],
        'calcArg': [_('The Calculate block returns a value calculated by an action.'), 'documentation', ''],
        'doArg': [_('The Do block is used to initiate an action.'), 'documentation', ''],
        'namedcalcArg': [_('The Calculate block returns a value calculated by an action.'), 'documentation', ''],
        'nameddoArg': [_('The Do block is used to initiate an action.'), 'documentation', ''],
        'namedcalc': [_('The Calculate block returns a value calculated by an action.'), 'documentation', ''],
        'calc': [_('The Calculate block returns a value calculated by an action.'), 'documentation', ''],
        'returnToUrl': [_('The Return to URL block will return a value to a webpage.'), 'documentation', ''],
        'return': [_('The Return block will return a value from an action.'), 'documentation', ''],
        // Boxes palette
        'storein': [_('The Store in block will store a value in a box.'), 'documentation', ''],
        ...
        'mod': [_('The Mod block returns the remainder from a division.'), 'documentation', ''],
        'int': [_('The Int block returns an integer.'), 'documentation', ''],
        // Boolean palette
        'not': [_('The Not block is the logical not operator.'), 'documentation', ''],
        'and': [_('The And block is the logical and operator.'), 'documentation', ''],
        'or': [_('The Or block is the logical or operator.'), 'documentation', ''],
        'boolean': [_('The Boolean block is used to specify true or false.'), 'documentation', ''],
        // Heap palette
        'push': [_('The Push block adds a value to the top of the heap.'), 'documentation', ''],
        'pop': [_('The Pop block removes the value at the top of the heap.'), 'documentation', ''],
        'setHeapEntry': [_('The Set-heap entry block sets a value in he heap at the specified location.'), 'documentation', ''],
        'indexHeap': [_('The Index-heap block returns a value in the heap at a specified location.'), 'documentation', ''],
        'reverseHeap': [_('The Reverse-heap block reverses the order of the heap.'), 'documentation', ''],
        'loadHeap': [_('The Load-heap block loads the heap from a file.'), 'documentation', ''],
        'saveHeap': [_('The Save-heap block saves the heap to a file.'), 'documentation', ''],
        'emptyHeap': [_('The Empty-heap block empties the heap.'), 'documentation', ''],
        'heapEmpty': [_('The Heap-empty? block returns true if the heap is empty.'), 'documentation', ''],
        'heapLength': [_('The Heap-length block returns the length of the heap.'), 'documentation', ''],
        'showHeap': [_('The Show-heap block displays the contents of the heap at the top of the screen.'), 'documentation', ''],
        'saveHeapToApp': [_('The Save-heap-to-app block saves the heap to a web page.'), 'documentation', ''],
        'loadHeapFromApp': [_('The Load-heap-from-app block loads the heap from a web page.'), 'documentation', ''],
        // Extras palette
        'comment': [_('The Comment block prints a comment at the top of the screen when the program is running in slow mode.'), 'documentation', ''],
        'wait': [_('The Wait block pauses the program for a specified number of seconds.'), 'documentation', ''],
        'vspace': [_('The Space block is used to add space between blocks.'), 'documentation', ''],
        'hspace': [_('The Space block is used to add space between blocks.'), 'documentation', ''],
        'openProject': [_('The Open project block is used to open a project from a web page.'), 'documentation', ''],
        'hideblocks': [_('The Hide blocks block hides the blocks.'), 'documentation', ''],
        'showblocks': [_('The Show blocks block shows the blocks.'), 'documentation', ''],
        'nobackground': [_('The No background block eliminates the background from the saved SVG output.'), 'documentation', '', 'makehelp'],
        'makeblock': [_('The Make block block creates a new block.'), 'documentation', '', 'makehelp'],
        ...
        'bezier': [_('The Bezier block draws a Bezier curve.'), 'documentation', ''],
        'controlpoint1': [_('The Control-point 1 block sets the first control point for the Bezier curve.'), 'documentation', ''],
        'controlpoint1': [_('The Control-point 2 block sets the second control point for the Bezier curve.'), 'documentation', ''],
        // Pen palette
        'sethue': [_('The Set hue block changes the color of the pen.'), 'documentation', ''],
        'setgrey': [_('The Set grey block changes the vividness of the pen color.'), 'documentation', ''],
        'settranslucency': [_('The Set translucency block changes the opacity of the pen.'), 'documentation', ''],
        'fill': [_('The Fill block fills in a shape with a color.'), 'documentation', null, 'fillhelp'],
        'hollowline': [_('The Hollow line block creates a line with a hollow center.'), 'documentation', ''],
        'background': [_('The Background block sets the window background color.'), 'documentation', ''],
        'grey': [_('The Grey block returns the current pen grey value.'), 'documentation', ''],
        'shade': [_('The Shade block returns the current pen shade value.'), 'documentation', ''],
        'pensize': [_('The Pen size block returns the current pen size value.'), 'documentation', ''],
        'setfont': [_('The Set font block sets the font used by the Show block.'), 'documentation', ''],
        // Media
        'tofrequency': [_('The To frequency block converts a pitch name and octave to Hertz.'), 'documentation', ''],
        'stopvideocam': [_('The Stop media block stops audio or video playback.'), 'documentation', ''],
        'loadFile': [_('The Open file block opens a file for use with the Show block.'), 'documentation', ''],
        'video': [_('The Video block selects video for use with the Show block.'), 'documentation', ''],
        'camera': [_('The Camera block connects a webcam to the Show block.'), 'documentation', ''],
        // Sensors palette
        'keyboard': [_('The Keyboard block returns computer keyboard input.'), 'documentation', ''],
        'toascii': [_('The To ASCII block converts numbers to letters.'), 'documentation', ''],
        'time': [_('The Time block returns the number of seconds that the program has been running.'), 'documentation', ''],
        'getcolorpixel': [_('The Get pixel block returns the color of the pixel under the mouse.'), 'documentation', ''],
        'getred': [_('The Get red block returns the red component of the pixel under the mouse.'), 'documentation', ''],
        'getgreen': [_('The Get green block returns the green component of the pixel under the mouse.'), 'documentation', ''],
        'getblue': [_('The Get blue block returns the blue component of the pixel under the mouse.'), 'documentation', ''],
        'loudness': [_('The Loudness block returns the volume detected by the microphone.'), 'documentation', ''],
        // Ensemble palette
        ...
        'xturtle': [_('The X mouse block returns the X position of the specified mouse.'), 'documentation', ''],
        'yturtle': [_('The Y mouse block returns the Y position of the specified mouse.'), 'documentation', ''],
        'setturtle': [_('The Set mouse block sends a stack of blocks to be run by the specified mouse.'), 'documentation', ''],
        'turtleheading': [_('The Mouse heading block returns the heading of the specified mouse.'), 'documentation', ''],
        'turtlecolor': [_('The Mouse color block returns the pen color of the specified mouse.'), 'documentation', ''],
        'startTurtle': [_('The Start mouse block starts the specified mouse.'), 'documentation', ''],
        'stopTurtle': [_('The Stop mouse block stops the specified mouse.'), 'documentation', ''],
    }
    ...
    if (beginnerMode && language === 'ja') {
        ...
        BLOCKHELP['if'] = [_('Conditionals lets your program take different actions depending on the condition.') + ' ' + _('In this example if the mouse button is pressed a snare drum will play.'), 'documentation', null, 'ifhelp'];
        BLOCKHELP['ifthenelse'] = [_('Conditionals lets your program take different actions depending on the condition.') + ' ' + _('In this example if the mouse button is pressed a snare drum will play.'), 'documentation', null, 'elifhelp'];
        BLOCKHELP['musickeyboard'] = [_('The Music keyboard block opens a piano keyboard that can be used to create notes.'), 'documentation', null, 'musickeyboardja'];
    }
};
