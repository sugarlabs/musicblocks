...
class Logo {
  ...
  setTurtleListener(turtle, listenerName, listener) {
    ...
    if (listenerName in tur.listeners) {
      this.activity.stage.removeEventListener(listenerName, tur.listeners[listenerName], false);
    }
      ...
      switch (logo.blockList[blk].name) {
        case "dectofrac":
          if (
            ...
            if (cblk === null) {
              ...
              if (typeof a === "number") {
                logo.blockList[blk].value = a < 0 ? "-" + mixedNumber(-a) : mixedNumber(a);
              } else {
        ...
        case "hue":
          if (
            ...
            logo.blockList[blk].value = logo.activity.turtles.getTurtle(turtle).painter.color;
          }
          break;
  ...
  updateNotation(note, duration, turtle, insideChord, drum, split) {
    ...
    const beatsIntoMeasure =
      ((tur.singer.notesPlayed[0] / tur.singer.notesPlayed[1] - tur.singer.pickup - durationTime) *
  ...
  doBreak(turtle) {
    ...
    for (let i = queueLength - 1; i > -1; i--) {
      if (
        ["forever", "repeat", "while", "until"].includes(this.blockList[turtle.queue[i].blk].name)
      ) {
        // while or until
  ...
  step() {
    for (const turtle in this.stepQueue) {
      if (this.stepQueue[turtle].length > 0) {
        if (turtle in this._unhighlightStepQueue && this._unhighlightStepQueue[turtle] != null) {
  ...
  runLogoCommands(startHere, env) {
    ...
    for (const turtle of this.activity.turtles.turtleList) {
      for (const listener in turtle.listeners) {
        this.activity.stage.removeEventListener(listener, turtle.listeners[listener], false);
      }
  ...
  runFromBlockNow(logo, turtle, blk, isflow, receivedArg, queueStart) {
    ...
    if (logo.blockList[blk].protoblock.args > 0) {
      for (let i = 1; i <= logo.blockList[blk].protoblock.args; i++) {
        if (logo.blockList[blk].protoblock.dockTypes[i] === "in") {
          ...
          args.push(
            logo.parseArg(logo, turtle, logo.blockList[blk].connections[i], blk, receivedArg)
          );
        }
        if (
    ...
    if (!logo.blockList[blk].isArgBlock()) {
      ...
      if (
        ...
        logo.activity.errorMsg("I do not know how to " + logo.blockList[blk].name + ".", blk);
      }
    ...
    if (nextBlock != null) {
      ...
      if (logo.turtleDelay !== 0) {
        for (const pblk in tur.parameterQueue) {
          logo.activity.blocks.updateParameterBlock(logo, turtle, tur.parameterQueue[pblk]);
        }
      }
        ...
        for (const b in tur.endOfClampSignals) {
          for (let i = 0; i < tur.endOfClampSignals[b].length; i++) {
            if (tur.endOfClampSignals[b][i] != null) {
              if (tur.butNotThese[b] == null || tur.butNotThese[b].indexOf(i) === -1) {
  ...
  async dispatchTurtleSignals(turtle, beatValue, blk, delay) {
    ...
    const __pen = (turtle, name, b, timeout) => {
      ...
      switch (name) {
        ...
        case "pendown":
          break;
        default:
          arg = this.parseArg(this, turtle, this.blockList[b].connections[1], b, this.receivedArg);
          break;
      }
    ...
    const __right = (turtle, b, waitTime, stepTime, sign) => {
      const arg =
        this.parseArg(this, turtle, this.blockList[b].connections[1], b, this.receivedArg) * sign;
      if (suppressOutput) {
        const savedPenState = tur.painter.penState;
        tur.painter.penState = false;
