...
function Logo () {
    ...
    this._clearParameterBlocks = function () {
        for (var blk = 0; blk < this.blocks.blockList.length; blk++) {
            if (this.blocks.blockList[blk].protoblock.parameter && this.blocks.blockList[blk].text !== null) {
    ...
    this._updateParameterBlock = function (that, turtle, blk) {
        ...
        if (this.blocks.blockList[blk].protoblock.parameter && this.blocks.blockList[blk].text !== null) {
            ...
            if ((typeof that.blocks.blockList[blk].protoblock.updateParameter) === 'function') {
                value = that.blocks.blockList[blk].protoblock.updateParameter(that, turtle, blk);
            } else {
    ...
    this._blockSetter = function (blk, value, turtle) {
        if (typeof(this.blocks.blockList[blk].protoblock.setter) === 'function') {
            this.blocks.blockList[blk].protoblock.setter(this, value, turtle, blk);
        } else {
    ...
    this._runFromBlockNow = function (that, turtle, blk, isflow, receivedArg, queueStart) {
        ...
        case 'playbwd':
            ...
            break;
        default:
            if ((typeof that.blocks.blockList[blk].protoblock.flow) === 'function') {
                let res = that.blocks.blockList[blk].protoblock.flow(args, that, turtle, blk, receivedArg, actionArgs, isflow);
                if (res) {
                    let [cf, cfc, ret] = res;
        ...
        if (nextBlock != null) {
                    ...
                    if (that.unhighlightQueue[turtle] === undefined) {
                        ...
                        setTimeout(function () {
                            if (!turtle in that.unhighlightQueue) {
                                console.debug('turtle ' + turtle + ' not found in unhighlightQueue');
                                return;
                            }
            ...
            __checkCompletionState = function () {
                if (!that.turtles.running() && queueStart === 0 && that.justCounting[turtle].length === 0) {
                    ...
                    console.debug('SETTING LAST NOTE TIMEOUT: ' + that.recording + ' ' + that.suppressOutput[turtle]);
    ...
    this._processNote = function (noteValue, blk, turtle, callback) {
        ...
        if (this.inCrescendo[turtle].length > 0 && this.crescendoDelta[turtle].length === 0) {
            ...
            for (var synth in this.synthVolume[turtle]) {
                ...
                console.debug(synth + '= ' + this.synthVolume[turtle][synth][len - 1]);
                this._playbackPush(turtle, [this.previousTurtleTime[turtle], 'setsynthvolume', synth, last(this.synthVolume[turtle][synth])]);
            ...
            __playnote = function () {
                    ...
                    if (obj[0] > 0) {
                        ...
                        if (that.justCounting[turtle].length === 0) {
                            if (notes.length === 0) {
                                ...
                                console.debug('notes to play: ' + notes + ' ' + obj[0] + '/' + obj[1] + notesInfo);
                            }
                        } else {
                            if (notes.length === 0) {
                                console.debug('notes to count: R ' + obj[0] + '/' + obj[1]);
                            } else {
                                console.debug('notes to count: ' + notes + ' ' + obj[0] + '/' + obj[1] + notesInfo);
                            }
                        }
                    }
                                ...
                                if (that.oscList[turtle][thisBlk].length > 0) {
                                    ...
                                    for (var d = 0; d < notes.length; d++) {
                                        if (notes[d] in that.pitchDrumTable[turtle]) {
                                            if (!that.suppressOutput[turtle]) {
                                                console.debug(that.glide[turtle].length);
                                            ...
                                            if (!that.suppressOutput[turtle]) {
                                                ...
                                                if (that.glide[turtle].length > 0) {
                                                    if (that.glideOverride[turtle] === 0) {
                                                        ...
                                                        console.debug('first glide note: ' + that.glideOverride[turtle] + ' ' + beatValueOverride);
                ...
                if (that.noteDrums[turtle][thisBlk].length > 0) {
                    ...
                    if (that.notePitches[turtle][thisBlk].length === 0) {
                        ...
                        if (obj[0] > 0) {
                            if (that.justCounting[turtle].length === 0) {
                                console.debug('drums to play ' + notes + ' ' + obj[0] + '/' + obj[1]);
                            } else {
                                console.debug('drums to count ' + notes + ' ' + obj[0] + '/' + obj[1]);
                            }
                        }
