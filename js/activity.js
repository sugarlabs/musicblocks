        ...
        */
        ...
        this._doOpenPlugin = () => {
            ...
            const choice = confirm(
                _("Load a built-in plugin?") +
                "\n\n" +
                _("(Cancel to upload a custom plugin file)")
            );
            if (choice) {
                const name = prompt(
                    _(
                        "Enter plugin name (maths, weather, accelerometer, facebook, nutrition, rodi):"
                    )
                );
                if (name) {
                    ...
                    reader.onload = () => {
                        ...
                        setTimeout(() => {
                            const obj = processRawPluginData(
                                that,
                                reader.result,
                                pluginFile && pluginFile.name ? "file:" + pluginFile.name : "file:local-file"
                            );
                            // Save plugins to local storage.
                            if (obj !== null) {
                                that.storage.plugins = preparePluginExports(that, obj);
