...
class Activity {
        ...
        */
        ...
        this.init = async () => {
            ...
            try {
                ...
                await this.snapshotStorage.init();
            } catch (e) {
                // eslint-disable-next-line no-console
                console.error("Failed to initialize snapshot storage:", e);
     ...
     * @param {string} description - Optional user description for the snapshot
     */
    async saveSnapshot(description) {
        if (!this.snapshotStorage) {
            // eslint-disable-next-line no-console
            console.error("Snapshot storage not initialized");
            this.textMsg(_("Snapshot storage not available"));
            return;
        }
        ...
        try {
            ...
            if (!projectId) {
                this.textMsg(_("No project loaded"));
                return;
            }

            const projectData = this.prepareExport();
            const snapshotId = await this.snapshotStorage.saveSnapshot(
                projectId,
                projectData,
                description || ""
            );

            this.textMsg(_("Snapshot saved successfully"));
            // eslint-disable-next-line no-console
            console.log("Snapshot saved:", snapshotId);
        } catch (e) {
            // eslint-disable-next-line no-console
            console.error("Failed to save snapshot:", e);
            this.errorMsg(_("Failed to save snapshot"));
        }
    }

    /**
     * Open the snapshot history widget
     */
    openSnapshotHistory() {
        if (!this.snapshotStorage) {
            // eslint-disable-next-line no-console
