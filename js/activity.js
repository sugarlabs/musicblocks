    ...
    constructor() {
        ...
        this._onResize = (force) => {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                this.trebleSharpBitmap[i].x =
                    this.canvas.width / (2 * this.turtleBlocksScale) - 600;
                this.trebleFlatBitmap[i].x = this.canvas.width / (2 * this.turtleBlocksScale) - 600;
                this.sopranoSharpBitmap[i].x =
                    this.canvas.width / (2 * this.turtleBlocksScale) - 600;
                this.sopranoFlatBitmap[i].x =
                    this.canvas.width / (2 * this.turtleBlocksScale) - 600;
        ...
        this._restoreTrash = () => {
            ...
            if (
                ...
                const actionArg =
                    this.blocks.blockList[this.blocks.blockList[thisBlock].connections[1]];
                if (actionArg !== null) {
                    let label;
        ...
        this.showContents = () => {
            ...
            // For the first time, when the help block is loaded in small viewports, don't display the palette.
            if (window.innerWidth <= 600) this.palettes.hide() ;
        ...
        this._makeButton = (name, label, x, y) => {
            ...
            const img = new Image();
            img.src =
                "data:image/svg+xml;base64," + window.btoa(unescape(encodeURIComponent(name)));
        ...
        this.init = async () => {
            ...
            for (let i = 0; i < 7; i++) {
                this.grandSharpBitmap[i] = this._createGrid(
                    "data:image/svg+xml;base64," +
                        window.btoa(unescape(encodeURIComponent(GRAND_G)))
                );
                this.grandFlatBitmap[i] = this._createGrid(
                    "data:image/svg+xml;base64," +
                        window.btoa(unescape(encodeURIComponent(GRAND_F)))
                );
                this.trebleSharpBitmap[i] = this._createGrid(
                    "data:image/svg+xml;base64," +
                        window.btoa(unescape(encodeURIComponent(TREBLE_G)))
                );
                this.trebleFlatBitmap[i] = this._createGrid(
                    "data:image/svg+xml;base64," +
                        window.btoa(unescape(encodeURIComponent(TREBLE_F)))
                );
