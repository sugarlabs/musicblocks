  ...
  constructor() {
    ...
    function repositionBlocks(activity) {
      ...
      dragGroups.forEach(group => {
        ...
        const rightmostX = Math.max(
          ...group.map(
            id => activity.blocks.blockList[id].container.x + activity.blocks.blockList[id].width
          )
        );
        ...
        setTimeout(() => {
          ...
          if (message.length < 4) {
            ...
            if (that.blocks.protoBlockDict.hasOwnProperty(protoName)) {
              that.palettes.dict[paletteName].makeBlockFromSearch(protoblk, protoName, newBlock => {
    ...
    this._deletePlugin = () => {
      if (this.palettes.activePalette !== null) {
        ...
        for (let i = 0; i < this.palettes.dict[this.palettes.activePalette].protoList.length; i++) {
    ...
    this._setupBlocksContainerEvents = () => {
      ...
      const closeAnyOpenMenusAndLabels = () => {
        ["wheelDiv", "contextWheelDiv", "helpfulWheelDiv", "textLabel", "numberLabel"].forEach(
          id => {
    ...
    this.doSearch = () => {
      ...
      if (this.blocks.protoBlockDict.hasOwnProperty(protoName)) {
        this.palettes.dict[paletteName].makeBlockFromSearch(protoblk, protoName, newBlock => {
    ...
    this.__makeNewNote = (octave, solf) => {
      const newNote = [
        [0, "newnote", 300 - this.blocksContainer.x, 300 - this.blocksContainer.y, [null, 1, 4, 8]],
        [1, "divide", 0, 0, [0, 2, 3]],
        [
          2,
          [
      ...
      if (this.blocks.activeBlock !== null) {
        ...
        if (
          ...
          this.blocks.blockList[this.blocks.blockList[bottom].connections[0]].name === "newnote"
        ) {
        ...
        if (docById("paste").style.visibility === "visible" && event.keyCode === RETURN) {
          ...
          switch (event.keyCode) {
            case END:
              ...
              this.blocksContainer.y = -this.blocks.bottomMostBlock() + this.canvas.height / 2;
              this.stage.update();
              break;
            case PAGE_UP:
            ...
            case HOME:
              ...
              if (this.palettes.mouseOver) {
                ...
                this.palettes.activePalette.scrollEvent(-this.palettes.activePalette.scrollDiff, 1);
              } else {
        ...
        if (actionArg !== null) {
          ...
          if (uniqueName !== actionArg) {
            ...
            for (let b = 0; b < this.blocks.dragGroup.length; b++) {
              ...
              if (
                ["nameddo", "nameddoArg", "namedcalc", "namedcalcArg"].includes(me.name) &&
    ...
    this._changeBlockVisibility = () => {
      ...
      if (this.blocks.visible) {
        ...
        this.palettes.hide();
        changeImage(this.hideBlocksContainer.children[0], SHOWBLOCKSBUTTON, HIDEBLOCKSFADEDBUTTON);
      } else {
        changeImage(this.hideBlocksContainer.children[0], HIDEBLOCKSFADEDBUTTON, SHOWBLOCKSBUTTON);
        this.blocks.showBlocks();
        this.palettes.show();
      }
    };
        ...
        organizeBlock[lineId].arrangedBlocks?.forEach(staff => {
          ...
          staff.voices.forEach(voice => {
            ...
            if (staffBlocksMap[lineId].baseBlocks.length != 0) {
              staffBlocksMap[lineId].baseBlocks[staffBlocksMap[lineId].baseBlocks.length - 1][0][
                staffBlocksMap[lineId].baseBlocks[staffBlocksMap[lineId].baseBlocks.length - 1][0]
                  .length - 4
              ][4][1] = blockId;
            }
          ...
          if (repeatId.start == 0) {
            staffBlocksMap[staffIndex].repeatBlock.push([
              ...
              [
                ...
                staffBlocksMap[staffIndex].nameddoArray[staffIndex][repeatId.end + 1] === null
                  ? null
            ...
            const prevnameddo = _searchIndexForMusicBlock(
              ...
              staffBlocksMap[staffIndex].baseBlocks[repeatId.start][0][currentnammeddo][4][0]
            );
            const afternamedo = _searchIndexForMusicBlock(
              staffBlocksMap[staffIndex].baseBlocks[repeatId.end][0],
              staffBlocksMap[staffIndex].baseBlocks[repeatId.start][0][currentnammeddo][4][1]
            );
            ...
            const currentBlockId =
              staffBlocksMap[staffIndex].baseBlocks[repeatId.start][0][currentnammeddo][0];
    ...
    this.errorMsg = (msg, blk, text, timeout) => {
      ...
      switch (msg) {
        ...
        case NOSTRINGERRORMSG:
          ...
          this.stage.setChildIndex(this.errorArtwork["notastring"], this.stage.children.length - 1);
          break;
        case EMPTYHEAPERRORMSG:
          this.errorArtwork["emptyheap"].visible = true;
          this.stage.setChildIndex(this.errorArtwork["emptyheap"], this.stage.children.length - 1);
          break;
        case NOSQRTERRORMSG:
          this.errorArtwork["negroot"].visible = true;
          this.stage.setChildIndex(this.errorArtwork["negroot"], this.stage.children.length - 1);
          break;
        case NOACTIONERRORMSG:
          ...
          this.stage.setChildIndex(this.errorArtwork["nostack"], this.stage.children.length - 1);
          break;
        case NOBOXERRORMSG:
          if (text === null) {
            text = "foo";
          ...
          this.stage.setChildIndex(this.errorArtwork["emptybox"], this.stage.children.length - 1);
          break;
        case ZERODIVIDEERRORMSG:
          this.errorArtwork["zerodivide"].visible = true;
          this.stage.setChildIndex(this.errorArtwork["zerodivide"], this.stage.children.length - 1);
          break;
        case NANERRORMSG:
          this.errorArtwork["notanumber"].visible = true;
          this.stage.setChildIndex(this.errorArtwork["notanumber"], this.stage.children.length - 1);
          break;
        case NOINPUTERRORMSG:
          this.errorArtwork["noinput"].visible = true;
          this.stage.setChildIndex(this.errorArtwork["noinput"], this.stage.children.length - 1);
          break;
        default:
    ...
    this._setupPaletteMenu = () => {
      ...
      if (!this.helpfulWheelItems.find(ele => ele.label === "Home [HOME]"))
        this.helpfulWheelItems.push({
          label: "Home [HOME]",
          icon: "imgsrc:data:image/svg+xml;base64," + window.btoa(base64Encode(GOHOMEFADEDBUTTON)),
          display: true,
          fn: findBlocks
        });
      ...
      if (!this.helpfulWheelItems.find(ele => ele.label === "Show/hide blocks"))
        this.helpfulWheelItems.push({
          ...
          icon: "imgsrc:data:image/svg+xml;base64," + window.btoa(base64Encode(SHOWBLOCKSBUTTON)),
          display: true,
      ...
      if (!this.helpfulWheelItems.find(ele => ele.label === "Expand/collapse blocks"))
        this.helpfulWheelItems.push({
          ...
          icon:
            "imgsrc:data:image/svg+xml;base64," + window.btoa(base64Encode(COLLAPSEBLOCKSBUTTON)),
          display: true,
      ...
      if (!this.helpfulWheelItems.find(ele => ele.label === "Decrease block size"))
        this.helpfulWheelItems.push({
          ...
          icon: "imgsrc:data:image/svg+xml;base64," + window.btoa(base64Encode(SMALLERBUTTON)),
          display: true,
          fn: doSmallerBlocks
        });

      this.largerContainer = createButton(BIGGERBUTTON, _("Increase block size"), doLargerBlocks);
      ...
      if (!this.helpfulWheelItems.find(ele => ele.label === "Grid"))
        this.helpfulWheelItems.push({
          label: "Grid",
          icon: "imgsrc:data:image/svg+xml;base64," + window.btoa(base64Encode(CARTESIANBUTTON)),
          display: true,
          fn: piemenuGrid
        });
      ...
      if (!this.helpfulWheelItems.find(ele => ele.label === "Collapse"))
        this.helpfulWheelItems.push({
          label: "Collapse",
          icon: "imgsrc:data:image/svg+xml;base64," + window.btoa(base64Encode(COLLAPSEBUTTON)),
          display: true,
    ...
    this.doHelpfulSearch = () => {
      ...
      $j("#helpfulSearch").autocomplete("instance")._renderItem = (ul, item) => {
        return $j("<li></li>")
          .data("item.autocomplete", item)
          .append(
            '<img src="' + item.artwork + '" height = "20px">' + "<a>" + " " + item.label + "</a>"
          )
    ...
    this._makeButton = (name, label, x, y) => {
      ...
      container.setAttribute(
        "style",
        "position: absolute; right:" + (document.body.clientWidth - x) + "px;  top: " + y + "px;"
      );
