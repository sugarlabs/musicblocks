                ...
                if (
                    ...
                    switch (myBlock.name) {
                        ...
                        case "drum": {
                            ...
                            // myBlock.value may be null or an index that doesn't yet
                            // have a matching turtle (e.g. when saveLocally fires during
                            // page load before turtles are initialised). Guard before
                            // calling getTurtle so we reach the fallback args instead of
                            // letting getTurtle throw "Turtle null not found".
                            // eslint-disable-next-line no-case-declarations
                            const turtleIndex = myBlock.value;
                            // eslint-disable-next-line no-case-declarations
                            const turtle =
                                turtleIndex !== null &&
                                    turtleIndex !== undefined &&
                                    turtleIndex >= 0 &&
                                    turtleIndex < this.turtles.getTurtleCount()
                                    ? this.turtles.getTurtle(turtleIndex)
                                    : null;
