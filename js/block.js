...
class Block {
      ...
      if (this.name === "start" || this.name === "drum") {
        ...
        for (let t = 0; t < this.activity.turtles.getTurtleCount(); t++) {
          if (this.activity.turtles.getTurtle(t).startBlock === this) {
            this.activity.turtles.getTurtle(t).resizeDecoration(scale, this.bitmap.image.width);
  ...
  generateArtwork(firstTime) {
    ...
    if (this.overrideName && this.name !== "outputtools") {
      if (
        ["namedbox", "storein2", "nameddo", "nameddoArg", "namedcalc", "namedcalcArg"].includes(
          this.name
        )
      ) {
  ...
  isClampBlock() {
    return (
      this.protoblock.style === "clamp" || this.isDoubleClampBlock() || this.isArgFlowClampBlock()
    );
  }
  ...
  _loadEventHandlers() {
    ...
    this.container.on("pressmove", event => {
      ...
      if (that?.name === "vspace" && that.blocks.blockList[that.connections[1]]?.name === "rest2") {
        return;
      }
        ...
        setTimeout(() => {
          moved =
            Math.abs(event.stageX / that.activity.getStageScale() - that.original.x) +
              Math.abs(event.stageY / that.activity.getStageScale() - that.original.y) >
              20 && !window.hasMouse;
          getInput = !moved;
        }, 200);
      }
      ...
      const dx = Math.round(event.stageX / that.activity.getStageScale() + that.offset.x - oldX);
      let dy = Math.round(event.stageY / that.activity.getStageScale() + that.offset.y - oldY);
  ...
  _changeLabel() {
    ...
    if (this.name === "text") {
      ...
      piemenuBasic(this, filterLabels, filterValues, selectedType, platformColor.piemenuBasic);
      ...
      if (this.blocks.octaveNumber(blk)) {
        ...
        if (cblk !== null) {
          ...
          if (cblk !== null && ["rhythm2", "stuplet"].includes(this.blocks.blockList[cblk].name)) {
    ...
    if (!this._usePiemenu()) {
      ...
      this.label.style.width =
        Math.round((selectorWidth * this.blocks.blockScale * this.protoblock.scale) / 2) + "px";
    ...
    if (this.name === "text" && c != null) {
      ...
      switch (cblock.name) {
        ...
        case "storein":
          ...
          if (cblock.connections[1] === this.blocks.blockList.indexOf(this) && closeInput) {
