  ...
  if (logo.inNoteBlock[turtle].length === 0) {
    ...
    for (var f = 0; f < logo.factorList[turtle].length; f++) {
      ...
      if (factor === Math.floor(factor)) {
        ...
        var eventName =
          "__beat_" + logo.factorList[turtle][f] + "_" + turtle + "__";
        logo.stage.dispatchEvent(eventName);
      }
    }
  }
  ...
  class MyNoteValueBlock extends ValueBlock {
    ...
    arg(logo, turtle, blk) {
      if (
        ...
        var value = 0;
        if (
          logo.noteValue[turtle][last(logo.inNoteBlock[turtle])] !== null &&
          logo.noteValue[turtle][last(logo.inNoteBlock[turtle])] !== undefined
        ) {
          ...
          if (logo.justCounting[turtle].length === 0) {
            ...
            if (lastNote != null && lastNote in logo.notePitches[turtle]) {
              ...
              for (
                var i = 0;
                i <
                logo.notePitches[turtle][last(logo.inNoteBlock[turtle])].length;
                i++
              ) {
