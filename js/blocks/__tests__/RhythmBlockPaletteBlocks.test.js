...
describe("setupRhythmBlockPaletteBlocks", () => {
    ...
    describe("Tuplet4Block", () => {
        ...
        it("should not throw ReferenceError for totalBeats when the dispatch listener runs", () => {
            ...
            expect(turtle.doWait).toHaveBeenCalledWith(0);
        });
        ...
        it("should handle empty tuplet clamp without producing NaN in doWait", () => {
            logo.inMatrix = false;
            logo.tuplet = {};
            logo.tupletParams = {};
            logo.tupletRhythms = {};
            ...
            const turtle = activity.turtles.ithTurtle(turtleIndex);
            turtle.singer.beatFactor = 1;
            turtle.doWait.mockClear();
            activity.blocks.blockList["blkTuplet4_empty"] = { name: "tuplet4" };

            const tuplet4Block = DummyFlowBlock.createdBlocks["tuplet4"];
            tuplet4Block.flow([2, 4, 88], logo, turtleIndex, "blkTuplet4_empty");
            ...
            expect(turtle.doWait).toHaveBeenCalledWith(0);
            expect(logo.tuplet[turtleIndex]).toBe(false);
            expect(logo.addingNotesToTuplet[turtleIndex]).toBe(false);
        });
    });
    ...
    describe("STupletBlock", () => {
        ...
        it("should process a simple tuplet outside matrix and outside tuplet clamp", () => {
            ...
            expect(turtle.doWait).toHaveBeenCalledWith(0.3);
        });
    });
});
