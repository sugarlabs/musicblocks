...
describe("setupToneBlocks", () => {
  ...
  describe("OscillatorBlock", () => {
    ...
    it("should not create synth if oscillator exists and show error", () => {
      ...
      expect(activity.errorMsg).toHaveBeenCalledWith("You are adding multiple oscillator blocks.");
  ...
  describe("PartialBlock", () => {
    it("should error and stop turtle if weight is out of range", () => {
      ...
      expect(activity.errorMsg).toHaveBeenCalledWith("Partial weight must be between 0 and 1.");
    ...
    it("should error if no harmonic context exists", () => {
      ...
      expect(activity.errorMsg).toHaveBeenCalledWith("Partial block should be used inside of a Weighted-partials block.");
    });
  });
  ...
  describe("HarmonicBlock", () => {
    it("should push block onto harmonic stack and set listener", () => {
      ...
      expect(logo.setDispatchBlock).toHaveBeenCalledWith("harmonicBlk", 0, expect.stringContaining("_harmonic_"));
  ...
  describe("DisBlock", () => {
    it("should call doDistortion and update distortion effect if in timbre", () => {
      ...
      expect(global.instrumentsEffects[turtle][logo.timbre.instrumentName].distortionActive).toBe(true);
      expect(logo.timbre.distortionEffect).toContain("disBlk");
      expect(logo.timbre.distortionParams).toContain(0.5 * 100);
      expect(global.instrumentsEffects[turtle][logo.timbre.instrumentName].distortionAmount).toEqual(40);
      expect(ret).toEqual([1, 1]);
    });
  });
  ...
  describe("TremoloBlock", () => {
    it("should call doTremolo and update tremolo effect if in timbre", () => {
      ...
      expect(global.instrumentsEffects[turtle][logo.timbre.instrumentName].tremoloActive).toBe(true);
      ...
      expect(global.instrumentsEffects[turtle][logo.timbre.instrumentName].tremoloFrequency).toEqual(10);
      expect(global.instrumentsEffects[turtle][logo.timbre.instrumentName].tremoloDepth).toEqual(50);
  ...
  describe("PhaserBlock", () => {
    it("should call doPhaser and update phaser effect if in timbre", () => {
      ...
      expect(Singer.ToneActions.doPhaser).toHaveBeenCalledWith(0.5, 3, 392, turtle, "phaserBlk");
      expect(global.instrumentsEffects[turtle][logo.timbre.instrumentName].phaserActive).toBe(true);
  ...
  describe("ChorusBlock", () => {
    it("should call doChorus and update chorus effect if in timbre", () => {
      ...
      expect(Singer.ToneActions.doChorus).toHaveBeenCalledWith(1.5, 3.5, 70, turtle, "chorusBlk");
      expect(global.instrumentsEffects[turtle][logo.timbre.instrumentName].chorusActive).toBe(true);
  ...
  describe("SetVoiceBlock", () => {
    it("should push valid voice into turtle's voices and set listener", () => {
      ...
      expect(logo.setDispatchBlock).toHaveBeenCalledWith("setVoiceBlk", turtle, expect.stringMatching("_setvoice_"));
    });
  ...
  describe("SetTimbreBlock", () => {
    ...
    it("should set timbre when inRhythmRuler is false and inSample is false", () => {
      ...
      expect(Singer.ToneActions.setTimbre).toHaveBeenCalledWith("customTimbre", turtle, "setTimbreBlk2");
    });
