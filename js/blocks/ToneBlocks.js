    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            if (logo.timbre.osc.length != 0) {
                ...
                logo.synth.createSynth(turtle, logo.timbre.instrumentName, oscillatorType, logo.timbre.synthVals);
            }
...
class DuoSynthBlock extends FlowBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            ...
            logo.synth.createSynth(turtle, logo.timbre.instrumentName, 'duosynth', logo.timbre.duoSynthParamVals);
...
class AMSynth extends FlowBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            ...
            logo.synth.createSynth(turtle, logo.timbre.instrumentName, 'amsynth', logo.timbre.amSynthParamvals);
...
class FMSynth extends FlowBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            ...
            logo.synth.createSynth(turtle, logo.timbre.instrumentName, 'fmsynth', logo.timbre.fmSynthParamvals);
...
class PartialBlock extends FlowBlock {
    ...
    flow(args, logo, turtle) {
        ...
        if (logo.inHarmonic[turtle].length > 0) {
            ...
            //.TRANS: partials are weighted components in a harmonic series
            logo.errorMsg(_('Partial block should be used inside of a Weighted-partials block.'));
        }
    }
}
...
class DisBlock extends FlowClampBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            ...
            logo.timbre.distortionParams.push(last(logo.distortionAmount[turtle]) * 100);
            instrumentsEffects[turtle][logo.timbre.instrumentName]['distortionAmount'] = distortion;
        }
...
class TremoloBlock extends FlowClampBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            ...
            logo.timbre.tremoloParams.push(last(logo.tremoloFrequency[turtle]));
            instrumentsEffects[turtle][logo.timbre.instrumentName]['tremoloFrequency'] = frequency;
            logo.timbre.tremoloParams.push(last(logo.tremoloDepth[turtle]) * 100);
...
class PhaserBlock extends FlowClampBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            ...
            instrumentsEffects[turtle][logo.timbre.instrumentName]['baseFrequency'] = baseFrequency;
        }
...
class ChorusBlock extends FlowClampBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            ...
            logo.timbre.chorusParams.push(last(logo.chorusDepth[turtle]) * 100);
...
class VibratoBlock extends FlowClampBlock {
    ...
    flow(args, logo, turtle, blk, receivedArg, actionArgs, isflow) {
        ...
        if (logo.inTimbre) {
            ...
            logo.timbre.vibratoParams.push(last(logo.vibratoIntensity[turtle]) * 100);
            instrumentsEffects[turtle][logo.timbre.instrumentName]['vibratoIntensity'] = logo.vibratoIntensity[turtle];
