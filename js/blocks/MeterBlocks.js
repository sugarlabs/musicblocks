...
class CurrentMeterBlock extends ValueBlock {
    ...
    arg(logo, turtle, blk) {
        if (logo.inStatusMatrix && logo.blocks.blockList[logo.blocks.blockList[blk].connections[0]].name === 'print') {
            ...
            return logo.beatsPerMeasure[turtle] + ':' + logo.noteValuePerBeat[turtle];
        }
    }
}
    ...
    arg(logo, turtle, blk) {
        if (logo.inStatusMatrix && logo.blocks.blockList[logo.blocks.blockList[blk].connections[0]].name === 'print') {
            ...
            if (logo.notesPlayed[turtle][0] / logo.notesPlayed[turtle][1] < logo.pickup[turtle]) {
                ...
                return Math.floor(((logo.notesPlayed[turtle][0] / logo.notesPlayed[turtle][1] - logo.pickup[turtle]) * logo.noteValuePerBeat[turtle]) / logo.beatsPerMeasure[turtle]) + 1;
            }
        }
    }
}
    ...
    arg(logo, turtle, blk) {
        if (logo.inStatusMatrix && logo.blocks.blockList[logo.blocks.blockList[blk].connections[0]].name === 'print') {
            ...
            if (logo.notesPlayed[turtle][0] / logo.notesPlayed[turtle][1] < logo.pickup[turtle]) {
                ...
                return (((logo.notesPlayed[turtle][0] / logo.notesPlayed[turtle][1] - logo.pickup[turtle]) * logo.noteValuePerBeat[turtle]) % logo.beatsPerMeasure[turtle]) + 1;
            }
        }
    }
}
...
class ElapsedNotes2Block extends LeftBlock {
    ...
    arg(logo, turtle, blk, receivedArg) {
        if (logo.inStatusMatrix && logo.blocks.blockList[logo.blocks.blockList[blk].connections[0]].name === 'print') {
            ...
            var cblk = logo.blocks.blockList[blk].connections[1];
            var notevalue = logo.parseArg(logo, turtle, cblk, blk, receivedArg);
            if (notevalue == null || notevalue === 0) {
                return 0;
            } else {
                return (logo.notesPlayed[turtle][0] / logo.notesPlayed[turtle][1]) / notevalue;
            }
        }
    }
}
        ...
        if (!(args[0] in logo.actions)) {
            ...
            var __listener = function (event) {
                if (logo.turtles.turtleList[turtle].running) {
                    var queueBlock = new Queue(logo.actions[args[0]], 1, blk);
                    ...
                    // from here.
                    if (isflow) {
                        logo._runFromBlockNow(logo, turtle, logo.actions[args[0]], isflow, receivedArg);
                    } else {
                        logo._runFromBlock(logo, turtle, logo.actions[args[0]], isflow, receivedArg);
                    }
                }
            };
    ...
    flow(args, logo, turtle, blk) {
        if (args.length === 2 && typeof(args[0]) === 'number' && typeof(args[1]) === 'number') {
            ...
            if (bpm < 30) {
                ...
                logo.errorMsg(obj[0] + '/' + obj[1] + ' ' + _('beats per minute must be greater than') + ' ' + target, blk);
                logo._masterBPM = 30;
            } else if (bpm > 1000) {
                var obj = rationalToFraction(args[1]);
                var target = 1000 * 0.25 / args[1];
                logo.errorMsg(_('maximum') + ' ' + obj[0] + '/' + obj[1] + ' ' + _('beats per minute is') + ' ' + target, blk);
...
class SetBPM2Block extends FlowClampBlock {
    constructor() {
        ...
        this.formBlock({
            ...
            args: 2, argLabels: [_('bpm'), _('beat value')],
            defaults: [90, 1 / 4]
        });
        this.makeMacro((x, y) => [
...
class MeterBlock extends FlowBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.insideMeterWidget)
            logo._meterBlock = blk;
        ...
        if (arg0 <= 0)
            logo.beatsPerMeasure[turtle] = 4;
        else logo.beatsPerMeasure[turtle] = arg0;

        if (arg1 <= 0)
            logo.noteValuePerBeat[turtle] = 4;
        else logo.noteValuePerBeat[turtle] = 1 / arg1;

        logo.notationMeter(turtle, logo.beatsPerMeasure[turtle], logo.noteValuePerBeat[turtle]);
    }
}
