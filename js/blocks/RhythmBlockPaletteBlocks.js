    ...
    flow(args, logo, turtle, blk, receivedArg, actionArgs, isflow) {
        ...
        if (logo.inMatrix || logo.tuplet) {
            ...
            if (logo.drumStyle[turtle].length > 0) {
                ...
                logo.inNoteBlock[turtle].push(blk);
                logo.notePitches[turtle][last(logo.inNoteBlock[turtle])] = ['G'];
                logo.noteOctaves[turtle][last(logo.inNoteBlock[turtle])] = [4];
                logo.noteCents[turtle][last(logo.inNoteBlock[turtle])] = [0];
            }
