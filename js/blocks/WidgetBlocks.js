...
class EnvelopeBlock extends FlowBlock {
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.inTimbre) {
            logo.timbre.synthVals['envelope']['attack'] = last(logo.attack[turtle]);
            logo.timbre.synthVals['envelope']['decay'] = last(logo.decay[turtle]);
            logo.timbre.synthVals['envelope']['sustain'] = last(logo.sustain[turtle]);
            logo.timbre.synthVals['envelope']['release'] = last(logo.release[turtle]);

            if (logo.timbre.env.length != 0) {
                logo.errorMsg(_('You are adding multiple envelope blocks.'));
            } else {
                // Create the synth for the instrument.
                logo.synth.createSynth(turtle, logo.timbre.instrumentName, logo.timbre.synthVals['oscillator']['source'], logo.timbre.synthVals);
            }

            logo.timbre.env.push(blk);
            logo.timbre.ENVs.push(Math.round(last(logo.attack[turtle]) * 100));
            logo.timbre.ENVs.push(Math.round(last(logo.decay[turtle]) * 100));
            logo.timbre.ENVs.push(Math.round(last(logo.sustain[turtle]) * 100));
            logo.timbre.ENVs.push(Math.round(last(logo.release[turtle]) * 100));
        }
    }
}
...
class FilterBlock extends FlowBlock {
    constructor() {
        ...
        this.setHelpString();
        this.formBlock({
            args: 3, defaults: [_('highpass'), -12, 392],
            argLabels: [
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (args.length === 3 && typeof(args[1]) === 'number') {
            ...
            if ([-12, -24, -48, -96].indexOf(args[1]) === -1) {
                //.TRANS: rolloff is the steepness of a change in frequency.
                logo.errorMsg(_('Rolloff value should be either -12, -24, -48, or -96 decibels/octave.'));
            }
            ...
            if (logo.inTimbre) {
                ...
                instrumentsFilters[turtle][logo.timbre.instrumentName].push({'filterType': filtertype, 'filterRolloff': rollOff, 'filterFrequency': freq});
...
class TemperamentBlock extends StackClampBlock {
    constructor() {
        super('temperament');
        this.setPalette('widgets');
        this.setHelpString([_('The Temperament tool is used to define custom tuning.'), 'documentation', null, 'temperament']);
    ...
    flow(args, logo, turtle, blk) {
        ...
        if (logo.blocks.blockList[logo.blocks.blockList[blk].connections[2]].name === 'pitch') {
            var pitchBlock = logo.blocks.blockList[logo.blocks.blockList[blk].connections[2]];
            var note = logo.blocks.blockList[pitchBlock.connections[1]].value;
            var octave = logo.blocks.blockList[pitchBlock.connections[2]].value;
...
class MeterWidgetBlock extends StackClampBlock {
    constructor() {
        super('meterwidget');
        this.setPalette('widgets');
        this.setHelpString([_('The Meter block opens a tool to select strong beats for the meter.'), 'documentation', null, 'meterwidget']);
...
class ModeWidgetBlock extends StackClampBlock {
    constructor() {
        ...
        this.setHelpString([_('The Custom mode block opens a tool to explore musical mode (the spacing of the notes in a scale).'), 'documentation', '']);
...
class TempoBlock extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.beginnerBlock(true);
        this.setHelpString([_('The Tempo block opens a metronome to visualize the beat.'), 'documentation', null, 'tempo']);
...
class PitchDrumMatrixBlock extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.setHelpString([_('The Pitch drum matrix is used to map pitches to drum sounds.'), 'documentation', '']);
    ...
    flow(args, logo, turtle, blk) {
        ...
        var __listener = function (event) {
            if (logo.pitchDrumMatrix.drums.length === 0 || logo.pitchDrumMatrix.rowLabels.length === 0) {
...
class PitchSliderBlock extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.beginnerBlock(true);
        this.setHelpString([_('The Pitch slider tool to is used to generate pitches at selected frequencies.'), 'documentation', '']);
...
class ChromaticBlock extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.setHelpString();
        this.formBlock({ name: _('chromatic keyboard'), canCollapse: true });
        this.makeMacro((x, y) => [
...
class PitchStaircaseBlock extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.setHelpString([_('The Pitch staircase tool to is used to generate pitches from a given ratio.'), 'documentation', null, 'pitchstaircase']);
...
class RhythmRuler2Block extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.beginnerBlock(true);
        this.setHelpString([_('The Rhythm Maker block opens a tool to create drum machines.'), 'documentation', null, 'rhythmruler2']);
        //.TRANS: widget for subdividing a measure into distinct rhythmic elements
        this.formBlock({ name: _('rhythm maker'), canCollapse: true });
        this.makeMacro((x, y) => [
...
class MatrixBlock extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.beginnerBlock(true);
        this.setHelpString([_('The Phrase Maker block opens a tool to create musical phrases.'), 'documentation', null, 'matrix']);
    ...
    flow(args, logo, turtle, blk) {
        ...
        var __listener = function (event) {
            if (logo.tupletRhythms.length === 0 || logo.pitchTimeMatrix.rowLabels.length === 0) {
                ...
                for (var i = 0; i < logo.tupletRhythms.length; i++) {
                    ...
                    case 'simple':
                        ...
                        break;
                    default:
                        logo.pitchTimeMatrix.addNotes(logo.tupletRhythms[i][1], logo.tupletRhythms[i][2]);
                        break;
                    }
                }
...
class StatusBlock extends StackClampBlock {
    constructor() {
        ...
        this.setPalette('widgets');
        this.beginnerBlock(true);
        this.setHelpString([_('The Status block opens a tool for inspecting the status of Music Blocks as it is running.'), 'documentation', null, 'status']);
