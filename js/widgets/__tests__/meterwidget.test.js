...
describe("Meter Widget", () => {
    ...
    test("should setup default strong beats for 2/4", () => {
        meterWidget._setupDefaultStrongWeakBeats(2, 0.25);

        expect(meterWidget._strongBeats[0]).toBe(true);
        expect(meterWidget._strongBeats[1]).not.toBe(true);
    });

    test("should setup default strong beats for 6/8", () => {
        meterWidget._setupDefaultStrongWeakBeats(6, 0.125);

        expect(meterWidget._strongBeats[0]).toBe(true);
        expect(meterWidget._strongBeats[3]).toBe(true);

        [1, 2, 4, 5].forEach(i => {
            expect(meterWidget._strongBeats[i]).not.toBe(true);
        });
    });

    test("should not set strong beats for unknown time signature", () => {
        meterWidget._setupDefaultStrongWeakBeats(5, 0.25);

        meterWidget._strongBeats.forEach(b => {
