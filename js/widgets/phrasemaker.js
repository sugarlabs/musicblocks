...
class PhraseMaker {
    ...
    init() {
        ...
        this._ = _;
        this.calcNoteValueToDisplay = calcNoteValueToDisplay;
        this.DEFAULTDRUM = DEFAULTDRUM;
        this.DEFAULTVOICE = DEFAULTVOICE;
        this.DEFAULTVOLUME = DEFAULTVOLUME;
        this.delayExecution = delayExecution;
        this.docById = docById;
        this.docBySelector = docBySelector;
        this.DRUMS = DRUMS;
        this.EIGHTHNOTEWIDTH = EIGHTHNOTEWIDTH;
        this.FLAT = FLAT;
        this.getDrumIcon = getDrumIcon;
        this.getDrumName = getDrumName;
        this.getNote = getNote;
        this.getTemperament = getTemperament;
        this.i18nSolfege = i18nSolfege;
        this.isCustomTemperament = isCustomTemperament;
        this.last = last;
        this.LCD = LCD;
        this.MATRIXSOLFEHEIGHT = MATRIXSOLFEHEIGHT;
        this.MATRIXSOLFEWIDTH = MATRIXSOLFEWIDTH;
        this.noteIsSolfege = noteIsSolfege;
        this.NOTESYMBOLS = NOTESYMBOLS;
        this.noteToFrequency = noteToFrequency;
        this.platformColor = platformColor;
        this.PREVIEWVOLUME = PREVIEWVOLUME;
        this.SHARP = SHARP;
        this.Singer = Singer;
        this.slicePath = slicePath;
        this.SOLFEGECONVERSIONTABLE = SOLFEGECONVERSIONTABLE;
        ...
        const turtle = this.activity.turtles.ithTurtle(0); // Get turtle 0
        ...
        for (let i = 0; i < this.rowLabels.length; i++) {
            ...
            drumName = this.getDrumName(this.rowLabels[i]);
            ...
            cell.style.height = Math.floor(this.MATRIXSOLFEHEIGHT * this._cellScale) + 1 + "px";
            cell.style.width = Math.floor(this.MATRIXSOLFEWIDTH * this._cellScale) + "px";
            cell.style.minWidth = Math.floor(this.MATRIXSOLFEWIDTH * this._cellScale) + "px";
            ...
            if (this.rowLabels[i] === "print") break;
                ...
                cell.innerHTML = `&nbsp;&nbsp;<img 
                        src="${this.getDrumIcon(this.rowLabels[i])}" 
            ...
            if (this.rowLabels[i] === "print") break;
                ...
                if (
                    this.noteIsSolfege(this.rowLabels[i]) &&
                    !this.isCustomTemperament(this.activity.logo.synth.inTemperament)
                ) {
                    cell.innerHTML = `${this.i18nSolfege(this.rowLabels[i])}${this.rowArgs[i]
                        .toString()
                        .sub()}`;
                    noteObj = this.getNote(
                        this.rowLabels[i],
                        this.rowArgs[i],
                        0,
                    ...
                    if (this.isCustomTemperament(this.activity.logo.synth.inTemperament)) {
                        noteObj = this.getNote(
                            this.rowLabels[i],
                            this.rowArgs[i],
                            0,
                        ...
                        const notes = this.getTemperament(this.activity.logo.synth.inTemperament);
                        let note = [];
        ...
        if (this.lyricsON) {
            ...
            for (let i = 0; i < this.activity.logo.tupletRhythms.length; i++) {
                ...
                inputCell.style.height =
                    Math.floor(this.MATRIXSOLFEHEIGHT * this._cellScale) + 1 + "px";
        ...
        this._noteValueLabel.style.height =
            Math.floor(1.5 * this.MATRIXSOLFEHEIGHT * this._cellScale) + "px";
        this._noteValueLabel.style.width =
            Math.floor(2 * this.MATRIXSOLFEWIDTH * this._cellScale) + "px";
            ...
            switch (label) {
                ...
                case "drum":
                    this.activity.blocks.loadNewBlocks([
                        ...
                        [1, ["drumname", { value: this.DEFAULTDRUM }], 0, 0, [0]]
                    ]);
                    rLabel = blockLabel;
                    rArg = -1;
                    break;
    ...
    _createMatrixGraphicsPieSubmenu(blockIndex, condition, blk) {
        ...
        const __enterArgValue = () => {
            this.blockValue =
                this._pitchWheel.navItems[this._pitchWheel.selectedNavItemIndex].title;
            this.docById("this.wheelnav-_exitWheel-title-1").children[0].textContent =
                this.blockValue;
    ...
    _createColumnPieSubmenu(index, condition, sortedClose) {
        ...
        if (!this._noteBlocks) {
            ...
            if (condition === "drumblocks") {
                this._pitchWheel.navigateWheel(
                    noteLabels.indexOf(
                        this.docBySelector('.labelcol[alt="' + index + '__drumblocks"]').innerText
                    )
                );
            } else {
        ...
        const __selectionChanged = () => {
            ...
            const drumName = this.getDrumName(this.rowLabels[index]);
            const BELLSETIDX = {
                C: 1,
                D: 2,
                E: 3,
            ...
            if (drumName != null) {
                ...
                cell.innerHTML = `${this.i18nSolfege(this.rowLabels[index])}${this.rowArgs[index]
                    .toString()
                    .sub()}`;
                noteObj = this.getNote(
        ...
        const __pitchPreview = () => {
            ...
            if (condition === "pitchblocks") {
                ...
                obj[0] = obj[0].replace(this.SHARP, "#").replace(this.FLAT, "b");
                this.activity.logo.synth.setMasterVolume(this.PREVIEWVOLUME);
                this.Singer.setSynthVolume(
                    this.activity.logo,
                    0,
                    this.DEFAULTVOICE,
                    this.PREVIEWVOLUME
                ...
                setTimeout(() => {
                    this.activity.logo.synth.setMasterVolume(this.DEFAULTVOLUME);
                    this.Singer.setSynthVolume(this.activity.logo, 0, label, this.DEFAULTVOLUME);
    ...
    _blockReplace(oldblk, newblk) {
        ...
        const c1 = this.last(this.activity.blocks.blockList[oldblk].connections);
    ...
    _addNotesBlockBetween(aboveBlock, block, topBlock) {
        ...
        if (topBlock) {
            ...
            belowBlock = this.last(this.activity.blocks.blockList[aboveBlock].connections);
    ...
    _removePitchBlock(blockNo) {
        ...
        const c1 = this.last(this.activity.blocks.blockList[blockNo].connections);
    ...
    _sort() {
        ...
        for (let i = 0; i < this.rowLabels.length; i++) {
            ...
            const frequencyKey = this.noteToFrequency(
        ...
        for (let i = 0; i < sortedList.length; i++) {
            ...
            if (i === 0) {
            ...
            } else if (i > 0 && obj[1] !== "hertz" && obj[1] === this.last(this.rowLabels)) {
                //eslint-disable-next-line no-console
                console.debug("skipping " + obj[1] + " " + this.last(this.rowLabels));
                this._sortedRowMap.push(this.last(this._sortedRowMap));
                ...
                //eslint-disable-next-line no-console
                console.debug("pushing " + obj[1] + " " + this.last(this.rowLabels));
                this._sortedRowMap.push(this.last(this._sortedRowMap) + 1);
                lastObj = i;
    ...
    _export() {
        ...
        for (let i = 0; i < this.rowLabels.length; i++) {
            ...
            if (drumName != null) {
                ...
                if (this.noteIsSolfege(this.rowLabels[i])) {
                    exportLabel.innerHTML = `${this.i18nSolfege(this.rowLabels[i])}${this.rowArgs[i]
                        .toString()
                        .sub()}`;
                } else {
    ...
    note2Solfege(note, index) {
        ...
        if (!["♭", "♯"].includes(note[1])) {
            octave = note[1];
            newNote = this.SOLFEGECONVERSIONTABLE[note[0]];
        } else {
            octave = note[2];
            newNote = this.SOLFEGECONVERSIONTABLE[note.substr(0, 2)];
        }
    ...
    addTuplet(param) {
        ...
        let noteValueToDisplay = this.calcNoteValueToDisplay(param[0][1], param[0][0]);
        ...
        if (!this._matrixHasTuplets) {
            ...
            labelCell.style.height =
                Math.floor(1.5 * this.MATRIXSOLFEHEIGHT * this._cellScale) + "px";
            labelCell.style.width = Math.floor(2 * this.MATRIXSOLFEWIDTH * this._cellScale) + "px";
            ...
            for (let i = 0; i < firstRow.cells.length; i++) {
                ...
                cell.style.height = Math.floor(this.MATRIXSOLFEHEIGHT * this._cellScale) + "px";
        ...
        for (let i = 0; i < numberOfNotes; i++) {
            ...
            cell.style.height = Math.floor(1.5 * this.MATRIXSOLFEHEIGHT * this._cellScale) + "px";
            ...
            obj = this.toFraction(numerator / (totalNoteInterval / tupletTimeFactor));

            if (obj[1] < 13) {
                if (this.NOTESYMBOLS != undefined && obj[1] in this.NOTESYMBOLS) {
                    cell.innerHTML = `${obj[0]}<br>&mdash;<br>${obj[1]}<br>
                        <img 
                            src="${this.NOTESYMBOLS[obj[1]]}" 
                            height="${(this.MATRIXSOLFEHEIGHT / 2) * this._cellScale}"
                        >`;
                } else {
            ...
            for (let j = 0; j < this.rowLabels.length; j++) {
                ...
                if (MATRIXGRAPHICS.indexOf(this.rowLabels[j]) != -1) {
                    ...
                    drumName = this.getDrumName(this.rowLabels[j]);
                    if (drumName === null) {
    ...
    _noteWidth(noteValue) {
        return Math.max(Math.floor(this.EIGHTHNOTEWIDTH * (8 / noteValue) * this._cellScale), 15);
    }
    ...
    addNotes(numBeats, noteValue) {
        let noteValueToDisplay = this.calcNoteValueToDisplay(noteValue, 1);
        ...
        for (let j = 0; j < numBeats; j++) {
            ...
            if (this._matrixHasTuplets) {
                ...
                cell.style.minWidth = cell.style.width;
                cell.style.maxWidth = cell.style.width;
                cell.height = Math.floor(1.5 * this.MATRIXSOLFEHEIGHT * this._cellScale) + "px";
                cell.style.height =
                    Math.floor(1.5 * this.MATRIXSOLFEHEIGHT * this._cellScale) + "px";
                cell.style.backgroundColor = this.platformColor.tupletBackground;
                cell.style.borderLeft = barStyle;
    ...
    _deleteRhythmBlock(blockToDelete) {
        if (this.last(this.activity.blocks.blockList[blockToDelete].connections) !== null) {
            this.activity.blocks.sendStackToTrash(
                this.activity.blocks.blockList[
                    this.last(this.activity.blocks.blockList[blockToDelete].connections)
                ]
            );
        }
    ...
    _mapNotesBlocks(blockName, withName) {
        ...
        while (this.last(myBlock.connections) != null) {
            bottomBlockLoop += 1;
            ...
            blk = this.last(myBlock.connections);
    ...
    recalculateBlocks() {
        ...
        for (let i = 1; i < this.activity.logo.tupletRhythms.length; i++) {
            if (this.activity.logo.tupletRhythms[i][2] === this.last(adjustedNotes)[0]) {
                startidx += 1;
            } else {
    ...
    _createpiesubmenu(noteToDivide, tupletValue, condition) {
        ...
        if (condition === "tupletvalue") {
            ...
            this._menuWheel.navItems[3].navigateFunction = () => {
                if (!flag) {
                    for (let i = 12; i < 19; i++) {
                        this.docById("this.wheelnav-wheelDivptm-title-3").children[0].textContent =
                            this.newNoteValue;
        ...
        if (this.playingNow) {
            ...
            for (let i = 0; i < note.length; i++) {
                if (typeof note[i] === "number") {
                    ...
                    drumName = this.getDrumName(note[i]);
                }
            ...
            if (note[0] !== "R" && pitchNotes.length > 0) {
                this._playChord(pitchNotes, this.Singer.defaultBPMFactor / noteValue);
            }
            ...
            for (let i = 0; i < synthNotes.length; i++) {
                this.activity.logo.synth.trigger(
                    0,
                    [Number(synthNotes[i])],
                    this.Singer.defaultBPMFactor / noteValue,
                ...
                if (cell.style.backgroundColor === "black") {
                    if (this.rowLabels[j] === "hertz") {
                        ...
                        if (
                            ...
                            if (this.isCustomTemperament(this.activity.logo.synth.inTemperament)) {
                                const notes = this.getTemperament(
                                    this.activity.logo.synth.inTemperament
                                );
    ...
    __playNote(time, noteCounter) {
        ...
        if (this.lyricsON) {
            this.activity.textMsg(this._lyrics[noteCounter], 3000);
        }
        ...
        }, this.Singer.defaultBPMFactor * 1000 * time + this.activity.logo.turtleDelay);
    }
    ...
    _setNoteCell(j, colIndex, cell, playNote) {
        ...
        if (this.rowLabels[j] === "hertz") {
            ...
            drumName = this.getDrumName(note);
            graphicsBlock = false;
        ...
        // Using the alt attribute to store the note value
        const noteValue = cell.getAttribute("alt") * this.Singer.defaultBPMFactor;
            ...
            if (note[0][0] === "R" || note[0][0] == undefined) {
                ...
                for (let j = 0; j < note[0].length; j++) {
                    ...
                    if (!isNaN(parseInt(note[0][j]))) {
                        ...
                        obj = note[0][j].split(": ");
                        drumName = this.getDrumName(note[0][j]);
                    }
                            ...
                            if (this.isCustomTemperament(this.activity.logo.synth.inTemperament)) {
                                ...
                                newStack.push([
                                    ...
                                    [
                                        "solfege",
                                        {
                                            value: this.SOLFEGECONVERSIONTABLE[note[0][j][0] + "♯"]
                                        }
                                    ],
                                    0,
                                    0,
