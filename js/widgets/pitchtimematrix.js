...
function PitchTimeMatrix() {
  ...
  this.init = function(logo) {
    ...
    for (var i = 0; i < this.rowLabels.length; i++) {
      ...
      if (drumName != null) {
        ...
        var blockLabel = this._logo.blocks.protoBlockDict[this.rowLabels[i]][
          "staticLabels"
        ][0];
    ...
    var __selectionChanged = async function(updatingArgs) {
      ...
      if (updatingArgs === undefined) {
        ...
        that._logo.blocks.loadNewBlocks([
          ...
          [1, ["number", { value: parseInt(that.xblockValue[0]) }], 0, 0, [0]],
          [2, ["number", { value: parseInt(that.yblockValue[0]) }], 0, 0, [0]]
        ]);
        ...
        var z =
          that._logo.blocks.blockList[argBlock].container.children.length - 1;
      ...
      if (MATRIXGRAPHICS2.indexOf(that.rowLabels[blockIndex]) !== -1) {
        var blockLabel =
          that._logo.blocks.protoBlockDict[that.rowLabels[blockIndex]][
            "staticLabels"
          ][0];
        cell.innerHTML =
          blockLabel +
    ...
    var __selectionChanged = async function(updatingArgs) {
      ...
      if (updatingArgs === undefined) {
        ...
        that._logo.blocks.loadNewBlocks([
          ...
          [1, ["number", { value: parseInt(that.blockValue) }], 0, 0, [0]]
        ]);
  ...
  this._createColumnPieSubmenu = function(index, condition, sortedClose) {
    ...
    var __selectionChanged = function() {
      ...
      var z =
        that._logo.blocks.blockList[noteLabelBlock].container.children.length -
        1;
      ...
      if (drumName != null) {
        ...
        cell.innerHTML =
          that.rowLabels[index] + that.rowArgs[index].toString().sub();
  ...
  this._export = function() {
    ...
    exportDocument.getElementById("downloadb1").href = this._generateDataURI(
      uriData
    );
  ...
  this._update = function(i, value, k, noteCase) {
    ...
    if (
      noteCase === "rhythm" ||
      noteCase === "stuplet" || (noteCase === "tupletnote" && value !== null)
    ) {
  ...
  this._updateTupletValue = function(
    ...
    if (oldTupletValue < newTupletValue) {
      ...
      for (var i = oldTupletValue; i < newTupletValue; i++) {
        this._logo.tupletRhythms[noteToDivide] = this._logo.tupletRhythms[
          noteToDivide
        ]
      ...
      for (var i = oldTupletValue; i > newTupletValue; i--) {
        this._logo.tupletRhythms[noteToDivide] = this._logo.tupletRhythms[
          noteToDivide
        ].slice(0, this._logo.tupletRhythms[noteToDivide].length - 1);
      }
      for (
        var j = 0;
        j < this._logo.tupletRhythms[noteToDivide].length - 2;
          ...
          if (obj == null) {
            ...
            if (note[0][j][1] === "♯") {
              ...
              if (this._logo.synth.inTemperament == "custom") {
                ...
                newStack.push([
                  thisBlock + 1,
                  ["solfege", { value: SOLFEGECONVERSIONTABLE[note[0][j][0]] }],
                  0,
                  0,
                  [thisBlock]
                ]);
