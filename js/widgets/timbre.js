...
function TimbreWidget() {
  ...
  this._update = function(i, value, k) {
    ...
    if (this.isActive["envelope"] === true && this.env[i] != null) {
      for (j = 0; j < 4; j++) {
        updateParams[j] = this._logo.blocks.blockList[this.env[i]].connections[
          j + 1
        ];
      }
    }
    ...
    if (this.isActive["filter"] === true && this.fil[i] != null) {
      for (j = 0; j < 3; j++) {
        updateParams[j] = this._logo.blocks.blockList[this.fil[i]].connections[
          j + 1
        ];
      }
    }
    ...
    if (this.isActive["oscillator"] === true && this.osc[i] != null) {
      for (j = 0; j < 2; j++) {
        updateParams[j] = this._logo.blocks.blockList[this.osc[i]].connections[
          j + 1
        ];
      }
    }
    ...
    if (this.isActive["vibrato"] === true && this.vibratoEffect[i] != null) {
      ...
      if (
        ...
        var topOfClamp = this._logo.blocks.blockList[last(this.vibratoEffect)]
          .connections[3];
  ...
  this._playNote = function(note, duration) {
    ...
    if (this.instrumentName in instrumentsFilters[0]) {
      ...
      console.debug(
        paramsEffects.vibratoIntensity + " " + paramsEffects.vibratoFrequency
      );
      this._logo.synth.trigger(
        0,
        note,
  ...
  this._addFilterListeners = function() {
    ...
    for (var f = 0; f < this.fil.length; f++) {
      ...
      docById("sel" + f).addEventListener("change", function(event) {
        ...
        var error = instrumentsFilters[0][that.instrumentName].filter(function(
          el
        ) {
      ...
      effectsName[i].onclick = function() {
        ...
        if (effectChosen === "Tremolo") {
          ...
          if (that.vibratoEffect.length > 0) {
            ...
            instrumentsEffects[0][that.instrumentName]["vibratoFrequency"] =
              1 / 16;
          }
