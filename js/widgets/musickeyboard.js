/**
 * @file This contains the prototype of the MusicKeyboard Widget.
 *
 * @copyright 2015 Jefferson Lee
 * @copyright 2018 Ritwik Abhishek
 * @copyright 2018,20 Walter Bender
 *
 * @license
...
/*
   global platformColor, docById, wheelnav, slicePath, logo, Singer, last, _, FIXEDSOLFEGE1, turtles
   SHARP, FLAT, noteToFrequency, EIGHTHNOTEWIDTH, MATRIXSOLFEHEIGHT, MATRIXSOLFEWIDTH,
   i18nSolfege, toFraction, blocks, getNote, PREVIEWVOLUME, DEFAULTVOICE, PITCHES3,
   SOLFEGENAMES, PITCHES2, PITCHES, NOTESSHARP, NOTESFLAT, convertFromSolfege, SOLFEGECONVERSIONTABLE
...
/**
 * @class
 * @classdesc pertains to setting up all features of the musickeyboard widget and its UI features.
 *
...
class MusicKeyboard {
    ...
    */
    ...
    /**
     * Initialises the MusicKeyboard widget.
     * @returns {void}
     */
    ...
    playAll() {
        ...
        if (this.playingNow) {
            ...
            this._playChord(
                notes,
                this.selectedNotes[0].duration,
                this.selectedNotes[0].voice
            );
     ...
     * @param {string} time
     * @param {HTMLElement} playButtonCell
     * @returns {void}
     */
    playOne(counter, time, playButtonCell) {
        setTimeout(() => {
            ...
            if (counter < this.selectedNotes.length) {
                ...
                if (
                    this.keyboardShown &&
                    this.selectedNotes[counter - 1].objId[0] !== null
                ) {
                    for (
                        let i = 0;
                        i < this.selectedNotes[counter - 1].noteOctave.length;
                        i++
                    ) {
                ...
                for (let i = 0; i < this.selectedNotes[counter].noteOctave.length; i++) {
                    if (
                        this.keyboardShown &&
                        this.selectedNotes[counter].objId[0] !== null
                    ) {
    ...
    _createTable() {
        ...
        for (let j = 0; j < this.selectedNotes.length; j++) {
            ...
            noteMaxWidth =
                this._noteWidth(Math.max.apply(Math, this.selectedNotes[j].duration)) * 2 +
                "px";
            n = this.layout.length;
            for (let i = 0; i < this.layout.length; i++) {
                row = docById("mkb" + i);
    ...
    _divideNotes(start, divideNoteBy) {
        ...
        this._notesPlayed = this._notesPlayed.reduce((prevValue, curValue) => {
            if (parseInt(curValue.startTime) === start) {
                ...
                const newcurValue = JSON.parse(JSON.stringify(curValue));
                newcurValue.duration = curValue.duration / divideNoteBy;
                prevValue = prevValue.concat([newcurValue]);
                let oldcurValue = newcurValue;
                for (let i = 0; i < divideNoteBy - 1; i++) {
                    const newcurValue2 = JSON.parse(JSON.stringify(oldcurValue));
