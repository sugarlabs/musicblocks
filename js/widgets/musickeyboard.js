...
function MusicKeyboard() {
  ...
  this._createTable = function() {
    ...
    for (var j = 0; j < selectedNotes.length; j++) {
      ...
      var noteMaxWidth =
        this._noteWidth(Math.max.apply(Math, selectedNotes[j].duration)) * 2 +
        "px";
      var n = this.layout.length;
      for (var i = 0; i < this.layout.length; i++) {
        var row = docById("mkb" + i);
    ...
    var __selectionChanged = function() {
      ...
      if (label === "pitch") {
        for (var i = 0; i < pitchLabels.length; i++) {
          if (
            pitchLabels[i].indexOf(last(that.layout).noteName) !== -1 ||
  ...
  this._createColumnPieSubmenu = function(index, condition) {
    ...
    var __hertzSelectionChanged = function() {
      ...
      cell.innerHTML =
        that.layout[index].noteName + that.layout[index].noteOctave.toString();
    ...
    var __selectionChanged = function() {
      ...
      if (temp1 in FIXEDSOLFEGE1) {
        var temp2 =
          FIXEDSOLFEGE1[temp1].replace(SHARP, "#").replace(FLAT, "b") + octave;
      } else {
  ...
  this._save = function() {
    ...
    var newStack = [
      [0, ["action", { collapsed: false }], 100, 100, [null, 1, null, null]],
