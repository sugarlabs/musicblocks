...
it("triggers specific beat event when beatList contains current beat", () => {
  ...
  expect(enqueue).toHaveBeenCalled();
  expect(activity.stage.dispatchEvent)
    .toHaveBeenCalledWith("__beat_2_0__");
});
...
it("triggers offbeat event when beatList contains 'offbeat' and beat > 1", () => {
  ...
  expect(enqueue).toHaveBeenCalled();
  expect(activity.stage.dispatchEvent)
    .toHaveBeenCalledWith("__offbeat_0__");
});
...
it("shows error when dot value is -1", () => {
  ...
  expect(activity.errorMsg)
    .toHaveBeenCalledWith("An argument of -1 results in a note value of 0.", 1);
