...
describe("setupToneActions", () => {
    ...
    beforeAll(() => {
        ...
        global.Mouse = {
            getMouseFromTurtle: (tur) => {
                if (tur) return { MB: { listeners: [] } };
                return null;
            }
        };
    });
    ...
    beforeEach(() => {
        ...
        activity = {
            ...
            logo: {
                setDispatchBlock: jest.fn((blk, turtle, name) => {}),
                setTurtleListener: jest.fn((turtle, name, callback) => {
                    listenerCallbacks[name] = callback;
                }),
                synth: {
                    loadSynth: jest.fn(),
                    createSynth: jest.fn(),
